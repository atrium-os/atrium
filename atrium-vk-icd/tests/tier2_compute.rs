...
//!     CS writes a vec4 to a StorageBuffer SSBO; asserts the
//!     bytes land in Tier2Backend::compute_output_bytes().
//!     Proves the CsMain `out_buffer` pointer is wired through
//!     to the dispatcher's per-dispatch SSBO and that the
//!     Cranelift backend maps `(Compute, StorageBuffer)` ->
//!     the out_buffer parameter correctly.
