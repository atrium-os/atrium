//! `examples/loader_graphics_unorm` — like
//! `loader_graphics_interp` but the per-vertex colour
//! attribute is `VK_FORMAT_R8G8B8A8_UNORM` (4 bytes per
//! vertex, 4 × u8 in source) instead of `R32G32B32_SFLOAT`.
//! Proves the daemon's `assemble_vertices` correctly
//! expands UNORM bytes to f32 lanes (`u8 / 255.0`) before
//! handing the assembled stream to the VS.
//!
//! Vertex layout: 12 B vec3 position + 4 B RGBA8 colour =
//! 16 B stride.  Same triangle + colours as the
//! interp rung (V0=red, V1=green, V2=blue), so pixel(4,4)
//! should land in the same barycentric blend that
//! interp produced -- (16, 80, 159, 255) -- now reached
//! through the UNORM decode path.
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = map as *mut u8;
        let verts: [([f32; 3], [u8; 4]); 3] = [
            ([-0.5, -0.5, 0.0], [255,   0,   0, 255]), // V0 red
            ([ 0.5, -0.5, 0.0], [  0, 255,   0, 255]), // V1 green
            ([ 0.0,  0.5, 0.0], [  0,   0, 255, 255]), // V2 blue
        ];
        for (i, (pos, col)) in verts.iter().enumerate() {
            ...
            for k in 0..4 { *p.add(base + 12 + k) = col[k]; }
        }
    }
    ...
    let attrs = [
        ...
        vk::VertexInputAttributeDescription {
            ...
            format: vk::Format::R8G8B8A8_UNORM, offset: 12,
        },
    ];
    ...
    // Same proof shape as the interp rung: all three RGB
    // channels contributed (none saturated, none zero), and
    // alpha = 255 since every vertex's A was 255.  Concrete
    // values mirror loader_graphics_interp's (16, 80, 159,
    // 255) because the source colours quantise back to the
    // same f32 lanes after `u8 / 255.0`.
    let ok = r > 0 && g > 0 && b > 0
        && r < 255 && g < 255 && b < 255
        && a == 255;
    ...
    if ok {
        println!("PASS: R8G8B8A8_UNORM vertex colours decoded + interpolated");
        0.into()
    } else {
