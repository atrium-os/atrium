//! `examples/loader_graphics_lines` — Rung XX: LineList
//! topology (one DDA segment per vertex pair).
//!
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        ...
        // pos.x, pos.y, r, g, b
        let verts: [[f32; 5]; 2] = [
            [-0.9, 0.0,  1.0, 0.0, 0.0], // left, red
            [ 0.9, 0.0,  0.0, 0.0, 1.0], // right, blue
        ];
        let mut i = 0;
        for v in verts { for f in v { p.add(i).write_unaligned(f); i += 1; } }
    }
    unsafe { device.unmap_memory(vmem); }
    println!("vbuf write 2 verts (line)          -> OK");
    ...
    let rgb = |x: usize, y: usize| -> (u8, u8, u8) {
        let i = (y * W as usize + x) * 4;
        (pixels[i], pixels[i + 1], pixels[i + 2])
    };
    // Count lit pixels in row 4 (the line's row).
    let row_lit = (0..W as usize)
        .filter(|&x| { let (r,g,b) = rgb(x, 4); (r | g | b) != 0 }).count();
    let left = rgb(0, 4);
    let right = rgb(7, 4);
    // Off-line pixel must stay clear.
    let off = rgb(4, 0);
    println!("row4 lit={row_lit}  left(0,4)={left:?}  right(7,4)={right:?}  off(4,0)={off:?}");

    // A horizontal line spans most of row 4; left end red-leaning,
    // right end blue-leaning (interpolated); off-line stays clear.
    let ok = row_lit >= 6
        && left.0 > left.2          // left: R > B
        && right.2 > right.0        // right: B > R
        && off == (0, 0, 0);
    ...
    if ok {
        println!("PASS: LineList rasterized a DDA segment with interpolated colour");
        0.into()
    } else {
        eprintln!("FAIL: line not rasterized/interpolated as expected \
                   (row_lit={row_lit} left={left:?} right={right:?} off={off:?})");
        1.into()
    }
}
