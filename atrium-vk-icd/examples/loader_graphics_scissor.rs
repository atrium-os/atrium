//! `examples/loader_graphics_scissor` — proves
//! `vkCmdSetScissor` actually clips the rasterised
//! triangle.  Viewport stays fullscreen so any "scissor
//! ignored" regression would paint the full triangle;
//! with the scissor applied, pixels outside the scissor
//! rect stay clear-black even where the triangle would
...
const W: u32 = 16;
const H: u32 = 16;
// Scissor clips to the right half (x in [8..16), y in [0..16)).
// The NDC triangle covers a wide band across the framebuffer
// centred on x ≈ W/2, so the left half is "would-be painted
// without scissor" and the right half is "painted-and-survives-
// scissor".
const SC_X: i32 = 8;
const SC_Y: i32 = 0;
const SC_W: u32 = 8;
...
fn main() -> std::process::ExitCode {
    ...
    let clear = [0u8, 0, 0, 0];
    // Triangle in 16x16: covers x in [4..12), y in [4..12).
    // Scissor keeps x in [8..16).  So:
    //   pixel(9, 8) -- inside triangle AND inside scissor -> red
    //   pixel(5, 8) -- inside triangle but scissor X<8    -> clear
    //   pixel(8, 7) -- inside scissor + triangle edge     -> red
    //   pixel(2, 6) -- outside both                       -> clear
    let kept     = px(9, 8);
    let clipped  = px(5, 8);
    let edge     = px(9, 7);
    let outside  = px(2, 6);
    println!("pixel(9,8) inside  scissor+triangle -> {:?} (want {:?})", kept,    red);
    println!("pixel(5,8) clipped by  scissor      -> {:?} (want {:?})", clipped, clear);
    println!("pixel(9,7) inside  scissor+triangle -> {:?} (want {:?})", edge,    red);
    println!("pixel(2,6) outside both             -> {:?} (want {:?})", outside, clear);
    let ok = kept    == red
          && clipped == clear
          && edge    == red
          && outside == clear;
    ...
    if ok {
        println!("PASS: scissor clipped left-half triangle pixels; right half survived");
        0.into()
    } else {
        eprintln!("FAIL: scissor mapping incorrect (left-half pixels painted, or right-half missed)");
        1.into()
    }
}
