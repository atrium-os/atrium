//! `examples/loader_graphics_array` — 2D-array texture
//! sampling.  Uploads a 2-layer array texture:
//!   layer 0: 1x1 red
//!   layer 1: 1x1 green
//! FS samples with a vec3 coord `(u, v, layer=1.0)` against
//! a `sampler2DArray`; the cranelift backend picks the
//! `atrium_tex_sample_2d_array` helper (3-lane coord, not
//! cube), which addresses `data + layer * slice_bytes`.
//! Layer 1 = green, so pixel(4, 4) ends up green.
//!
//! Discriminator: pixel(4, 4) = green (0, 255, 0, 255).
//! Pre-fix, `ImageStorage` only held one layer's worth of
//! pixels, `array_layers` wasn't plumbed from the create
//! payload, and `TexDesc.depth` / `slice_bytes` were
//! always (1, 0) so the array helper's `layer * slice_bytes`
//! offset was zero -> always sampled layer 0 (red).
//!
...
fn build_textured_fs() -> Vec<u8> {
    ...
    // sampler2DArray = combined image+sampler with Arrayed=1
    // at descriptor set 0, binding 0.
    let img_ty = b.type_image(f32_ty, Dim::Dim2D, 0, /*arrayed*/1, 0, 1, ImageFormat::Unknown, None);
    ...
    // Constant layer index 1.0 -> samples array layer 1.
    let c_layer = b.constant_bit32(f32_ty, 1.0f32.to_bits());
    let main = b.begin_function(void, None, FunctionControl::NONE, void_fn).unwrap();
    b.begin_block(None).unwrap();
    let uv = b.load(vec2, None, in_uv, None, vec![]).unwrap();
    let u = b.composite_extract(f32_ty, None, uv, vec![0]).unwrap();
    let v = b.composite_extract(f32_ty, None, uv, vec![1]).unwrap();
    // 3-lane coord (u, v, layer): cranelift selects the
    // array helper from coord_lanes >= 3.
    let coord3 = b.composite_construct(vec3, None, vec![u, v, c_layer]).unwrap();
    let sampler_val = b.load(sampled_ty, None, sampler_var, None, vec![]).unwrap();
    let color = b.image_sample_implicit_lod(vec4, None, sampler_val, coord3, None, vec![]).unwrap();
...
fn main() -> std::process::ExitCode {
    ...
    let tex_view = unsafe { device.create_image_view(
        &vk::ImageViewCreateInfo::default()
            .image(tex_img).view_type(vk::ImageViewType::TYPE_2D_ARRAY)
            .format(vk::Format::R8G8B8A8_UNORM)
            .subresource_range(vk::ImageSubresourceRange {
                aspect_mask: vk::ImageAspectFlags::COLOR,
                base_mip_level: 0, level_count: 1,
                base_array_layer: 0, layer_count: 2,
            }),
    ...
    unsafe {
        let p = sm as *mut u8;
        // Layer 0 = red (offset 0), layer 1 = green (offset 4).
        let l0: [u8; 4] = [255, 0, 0, 255];
        let l1: [u8; 4] = [0, 255, 0, 255];
        for k in 0..4 { *p.add(k) = l0[k]; }
        for k in 0..4 { *p.add(4 + k) = l1[k]; }
    }
    ...
    // Upload both array layers (one region each).
    ...
    let tex_copy_l1 = vk::BufferImageCopy::default()
        .buffer_offset(4).buffer_row_length(0).buffer_image_height(0)
        .image_subresource(vk::ImageSubresourceLayers {
            aspect_mask: vk::ImageAspectFlags::COLOR,
            mip_level: 0, base_array_layer: 1, layer_count: 1,
        })
        .image_offset(vk::Offset3D { x: 0, y: 0, z: 0 })
        .image_extent(vk::Extent3D { width: TEX_W, height: TEX_H, depth: 1 });
    unsafe {
    ...
    let b = pixels[i + 2];
    let a = pixels[i + 3];
    println!("pixel(4,4) = ({r},{g},{b},{a})  (want green 0,255,0,255 from array layer 1)");
    // FS sampled array layer 1 (green) via the 3-lane coord.
    // The single-texel layers mean no filtering ambiguity:
    // pure green.  Layer 0 (red) would mean the slice offset
    // wasn't applied.
    let ok = r == 0 && g == 255 && b == 0 && a == 255;
    ...
    if ok {
        println!("PASS: sampler2DArray fetched layer 1's green texel");
        0.into()
    } else {
        eprintln!("FAIL: pixel(4,4) not green -- array layer 1 wasn't fetched");
        1.into()
    }
}
