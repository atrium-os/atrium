//! `examples/loader_graphics_cube` — cubemap sampling.
//! Uploads a 6-face cube texture (1x1 each, distinct
//! colours per face) and samples it with a direction
//! vector pointing at +Z (face 4).  FS uses a `samplerCube`
//! (image Dim=Cube); cranelift picks the
//! `atrium_tex_sample_cube` helper, which does major-axis
//! face selection + sc/tc/ma projection and reads
//! `data + face * slice_bytes`.
//!
//! Face colour map (Vulkan face order
//! 0=+X 1=-X 2=+Y 3=-Y 4=+Z 5=-Z):
//!   +X red    -X green  +Y blue
//!   -Y yellow +Z magenta -Z cyan
//! Direction (0, 0, 1) -> +Z = face 4 = magenta.
//!
//! Discriminator: pixel(4, 4) = magenta (255, 0, 255, 255).
//! Reuses the layered-image plumbing from Rung GG with
//! depth=6.
//!
...
fn build_textured_fs() -> Vec<u8> {
    ...
    // samplerCube = combined image+sampler with Dim=Cube
    // at descriptor set 0, binding 0.  (in_uv is loaded but
    // unused -- the sample direction is a constant.)
    let _ = in_uv;
    let img_ty = b.type_image(f32_ty, Dim::DimCube, 0, 0, 0, 1, ImageFormat::Unknown, None);
    ...
    // Constant direction (0, 0, 1) -> +Z = face 4.
    let c0 = b.constant_bit32(f32_ty, 0.0f32.to_bits());
    let c1 = b.constant_bit32(f32_ty, 1.0f32.to_bits());
    let dir = b.constant_composite(vec3, vec![c0, c0, c1]);
    let main = b.begin_function(void, None, FunctionControl::NONE, void_fn).unwrap();
    b.begin_block(None).unwrap();
    let sampler_val = b.load(sampled_ty, None, sampler_var, None, vec![]).unwrap();
    let color = b.image_sample_implicit_lod(vec4, None, sampler_val, dir, None, vec![]).unwrap();
...
fn main() -> std::process::ExitCode {
    ...
    let tex_view = unsafe { device.create_image_view(
        &vk::ImageViewCreateInfo::default()
            .image(tex_img).view_type(vk::ImageViewType::CUBE)
            .format(vk::Format::R8G8B8A8_UNORM)
    ...
    unsafe {
        let p = sm as *mut u8;
        // 6 faces, one RGBA8 texel each at offset face*4.
        // Vulkan face order 0=+X 1=-X 2=+Y 3=-Y 4=+Z 5=-Z.
        let faces: [[u8; 4]; 6] = [
            [255,   0,   0, 255], // +X red
            [  0, 255,   0, 255], // -X green
            [  0,   0, 255, 255], // +Y blue
            [255, 255,   0, 255], // -Y yellow
            [255,   0, 255, 255], // +Z magenta  <- sampled
            [  0, 255, 255, 255], // -Z cyan
        ];
        for (f, c) in faces.iter().enumerate() {
            for k in 0..4 { *p.add(f * 4 + k) = c[k]; }
        }
    }
    ...
    // Upload all six cube faces (one region per face).
    let face_copies: Vec<vk::BufferImageCopy> = (0..6u32).map(|f| {
        vk::BufferImageCopy::default()
            .buffer_offset((f * 4) as u64)
            .buffer_row_length(0).buffer_image_height(0)
            .image_subresource(vk::ImageSubresourceLayers {
                aspect_mask: vk::ImageAspectFlags::COLOR,
                mip_level: 0, base_array_layer: f, layer_count: 1,
            })
    ...
    let b = pixels[i + 2];
    let a = pixels[i + 3];
    println!("pixel(4,4) = ({r},{g},{b},{a})  (want magenta 255,0,255,255 from +Z face 4)");
    // Direction (0,0,1) -> +Z = face 4 = magenta.  Any other
    // colour means the cube major-axis face selection or the
    // `face * slice_bytes` offset went wrong.
    let ok = r == 255 && g == 0 && b == 255 && a == 255;
    ...
    if ok {
        println!("PASS: samplerCube selected +Z face 4 (magenta)");
        0.into()
    } else {
        eprintln!("FAIL: pixel(4,4) not magenta -- cube face selection/offset wrong");
        1.into()
    }
}
