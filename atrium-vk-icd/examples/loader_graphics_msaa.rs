//! `examples/loader_graphics_msaa` — 4x MSAA (coverage-
//! resolved).  Renders a pure-red triangle over a black
//! clear with a pipeline whose rasterizationSamples = 4.
//! The rasterizer tests 4 sub-pixel sample points per pixel
//! and blends the fragment colour with the destination by
//! the covered fraction, so triangle-EDGE pixels come out
//! as intermediate reds (0 < R < 255) -- antialiased.
//!
//! Discriminator: at least one pixel has 0 < R < 255 (a
//! partial-coverage edge), the interior is full red, and a
//! corner is black.  Without MSAA every pixel is binary
//! (R = 0 or 255); the presence of partial-R pixels proves
//! the coverage resolve.
//!
...
fn main() -> std::process::ExitCode {
    ...
    let fs_spv = build_constant_color_fs([1.0, 0.0, 0.0, 1.0]);
    ...
    // Black opaque clear so edge pixels show a partial RED
    // (the coverage blend) while alpha stays 255.
    ...
    // ── Assert MSAA coverage produced antialiased edges. ────────
    let px = |x: usize, y: usize| -> [u8; 4] {
        let i = (y * W as usize + x) * 4;
        [pixels[i], pixels[i+1], pixels[i+2], pixels[i+3]]
    };
    // Count pixels with a partial red (0 < R < 255) -- only
    // possible from sub-pixel coverage blending.
    let mut partial = 0usize;
    for i in 0..(W * H) as usize {
        let r = pixels[i * 4];
        if r > 0 && r < 255 { partial += 1; }
    }
    let center = px(3, 3);   // triangle interior -> full red
    let corner = px(0, 0);   // outside -> black clear
    println!("partial-coverage edge pixels (0<R<255): {partial}");
    println!("pixel(3,3) interior = {:?} (want full red)", center);
    println!("pixel(0,0) corner   = {:?} (want black)", corner);
    // MSAA proof: edges are antialiased (>=1 partial pixel),
    // interior is solid red, corner is the black clear.
    let ok = partial >= 1
        && center == [255, 0, 0, 255]
        && corner == [0, 0, 0, 255];
    ...
    if ok {
        println!("PASS: 4x MSAA produced antialiased (partial-coverage) edges");
        0.into()
    } else {
        eprintln!("FAIL: no antialiased edges (MSAA coverage not applied?)");
        1.into()
    }
}
