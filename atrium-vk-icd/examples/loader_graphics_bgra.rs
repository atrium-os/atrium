...
//! [BGRA variant] UVs interpolated across the triangle.
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = sm as *mut u8;
        // 2x2 texture in B8G8R8A8 byte order; all four texels
        // encode RED -> bytes [B=0, G=0, R=255, A=255].  If the
        // daemon honours the BGRA format it samples R from byte 2
        // (=255) and B from byte 0 (=0), yielding red.  If it
        // wrongly treated the texels as RGBA the sample would be
        // BLUE (byte 2 -> B).
        let texels: [[u8; 4]; 4] = [
            [0, 0, 255, 255],
            [0, 0, 255, 255],
    ...
    // Every texel encodes RED in BGRA byte order.  A daemon
    // that honours the B8G8R8A8_UNORM format samples it as red
    // (R high, B ~0).  A daemon that ignored the format and read
    // the bytes as RGBA would produce BLUE (R ~0, B high).
    let ok = r > 200 && b < 60 && a == 255;
    ...
    if ok {
        println!("PASS: BGRA texture sampled with R/B swap -> red");
        0.into()
    } else {
        eprintln!("FAIL: BGRA sample not red (R={r} G={g} B={b} A={a}) -- format ignored?");
        1.into()
    }
}
