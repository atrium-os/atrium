//! `examples/loader_graphics_shadow` — depth-comparison
//! (shadow-map / PCF-style) sampling via
//! `OpImageSampleDrefImplicitLod`.
//!
//! The FS declares a `sampler2DShadow` (OpTypeImage with
//! Depth=1) and samples it with a depth reference
//! `dref = 0.75`.  The stored depth texture holds R = 0.5
//! (128/255).  The atrium-spv frontend lowers the Dref
//! opcode to `r <= dref ? 1.0 : 0.0`, so 0.5 <= 0.75
//! yields 1.0 ("lit").  The FS outputs (cmp, 0, 0, 1), so
//! a lit result reads back as pure red (255, 0, 0, 255).
//!
//! Discriminator: pixel(4, 4) = red (255, 0, 0, 255).
//! A raw (non-comparison) sample would have returned the
//! stored R = 0.5 -> (128, 0, 0, 255), so red vs. dark-red
//! distinguishes "Dref compare applied" from "sampler
...
fn build_textured_fs() -> Vec<u8> {
    ...
    let c_dref = b.constant_bit32(f32_ty, 0.75f32.to_bits());
...
fn main() -> std::process::ExitCode {
    ...
    let b = pixels[i + 2];
    let a = pixels[i + 3];
    println!("pixel(4,4) = ({r},{g},{b},{a})  (want lit red 255,0,0,255 from Dref compare)");
    // Dref compare: stored R=0.5 <= dref=0.75 -> 1.0 (lit)
    // -> output (1.0, 0, 0, 1) -> red.  A raw sample would
    // yield R=128 (dark red); the comparison must promote
    // it to 255.
    ...
    if ok {
        println!("PASS: shadow Dref compare (0.5 <= 0.75) returned lit red");
        0.into()
    } else {
        eprintln!("FAIL: pixel(4,4) not lit red -- Dref comparison not applied");
        1.into()
    }
}
