//! `examples/loader_graphics_depth_bounds` — proves the
//! depth bounds test is honoured on both axes.  Pipeline
//! enables the bounds test with range [0.0, 0.5]; the
//! depth attachment is cleared to 1.0 (the round-trip
//! clear value).  Per Vulkan spec, the bounds test compares
//! the EXISTING depth-buffer value (1.0) against the range
//! and discards out-of-range fragments.  1.0 > 0.5, so
//! every fragment is rejected -- pixel(3,3) stays
//! clear-black despite the depth test (LESS) on its own
//! would have let the front triangle paint.
//!
//! Pre-fix, depthBoundsTestEnable / minDepthBounds /
//! maxDepthBounds were not extracted from the create info
//! and the rasterizer had no bounds-test gate; pixel ended
//! up green (front triangle painted normally).
//!
//! Scene:
//!
...
fn main() -> std::process::ExitCode {
    ...
    // Depth bounds test ON with range [0.0, 0.5].  The
    // attachment is cleared to 1.0 (see clears[] below); the
    // bounds test compares the EXISTING buffer value (1.0)
    // against [0.0, 0.5] and discards every fragment that
    // would otherwise have passed the LESS depth compare.
    ...
    // Cleared depth = 1.0 is outside [0.0, 0.5], so the
    // bounds test must reject every fragment for both
    // triangles -- pixel stays at the colour clear value.
    let i = (3 * W as usize + 3) * 4;
    let got = [pixels[i], pixels[i+1], pixels[i+2], pixels[i+3]];
    let want_clear = [0u8, 0, 0, 0];
    let green      = [51u8, 255, 51, 255];
    println!("pixel(3,3) = {:?}  (want clear {:?}, NOT green {:?})",
        got, want_clear, green);
    let ok = got == want_clear;
    ...
    if ok {
        println!("PASS: depth bounds test rejected every fragment (existing=1.0 vs [0.0, 0.5])");
        0.into()
    } else {
        eprintln!("FAIL: depth bounds test ignored -- front triangle painted green");
        1.into()
    }
}
