//! `examples/loader_graphics_frontface` — Rung VV:
...
fn build_fs() -> Vec<u8> {
    ...
    let vec4 = b.type_vector(f32_ty, 4);
    let void_fn = b.type_function(void, vec![]);
    let ptr_in_bool = b.type_pointer(None, StorageClass::Input, bool_ty);
    let ptr_out_vec4 = b.type_pointer(None, StorageClass::Output, vec4);
    // gl_FrontFacing: Input bool decorated BuiltIn FrontFacing.
    let ff_var = b.variable(ptr_in_bool, None, StorageClass::Input, None);
    b.decorate(ff_var, Decoration::BuiltIn,
        vec![rspirv::dr::Operand::BuiltIn(BuiltIn::FrontFacing)]);
    ...
    let main = b.begin_function(void, None, FunctionControl::NONE, void_fn).unwrap();
    b.begin_block(None).unwrap();
    let ff = b.load(bool_ty, None, ff_var, None, vec![]).unwrap();
    // g = ff ? 1 : 0 ; r = ff ? 0 : 1
    let g = b.select(f32_ty, None, ff, c_one_f, c_zero_f).unwrap();
    let r = b.select(f32_ty, None, ff, c_zero_f, c_one_f).unwrap();
    let rgba = b.composite_construct(vec4, None,
        vec![r, g, c_zero_f, c_one_f]).unwrap();
    b.store(out, rgba, None, vec![]).unwrap();
    b.ret().unwrap();
    b.end_function().unwrap();
    b.entry_point(ExecutionModel::Fragment, main, "main", vec![ff_var, out]);
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        ...
        // Tri A (left half): order P,Q,R.
        // Tri B (right half): order reversed (P,R,Q) -> opposite winding.
        let verts: [[f32; 2]; 6] = [
            [-0.9, -0.8], [-0.1, -0.8], [-0.5, 0.8], // A
            [ 0.1, -0.8], [ 0.5, 0.8], [ 0.9, -0.8], // B (reversed)
        ];
        let mut i = 0;
    ...
    let (ar, ag) = rg(2, 3);
    let (br, bg) = rg(6, 3);
    println!("triA(2,3) = (R={ar},G={ag})   triB(6,3) = (R={br},G={bg})");

    let a_green = ag > ar && ag > 0;
    let a_red   = ar > ag && ar > 0;
    let b_green = bg > br && bg > 0;
    let b_red   = br > bg && br > 0;
    // Exactly one triangle green, the other red.
    let ok = (a_green && b_red) || (a_red && b_green);
    ...
    if ok {
        println!("PASS: opposite-winding triangles distinguished by gl_FrontFacing \
                  (one green, one red)");
        0.into()
    } else {
        eprintln!("FAIL: gl_FrontFacing did not split front/back \
                   (A R={ar} G={ag}, B R={br} G={bg})");
        1.into()
    }
}
