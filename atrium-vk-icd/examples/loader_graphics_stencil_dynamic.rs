//! `examples/loader_graphics_stencil_dynamic` — same
//! two-pass stencil masking as `loader_graphics_stencil`
//! (Rung BB) but with EVERY stencil field driven by
//! `vkCmd*` dynamic-state calls.  Pipelines declare
//! `stencilTestEnable = false` and all-default ops;
//! mid-cmdbuf the app issues the full pile:
//!   vkCmdSetStencilTestEnable(true)
//!   vkCmdSetStencilOp(FRONT_AND_BACK, ...)
//!   vkCmdSetStencilCompareMask(FRONT_AND_BACK, 0xff)
//!   vkCmdSetStencilWriteMask  (FRONT_AND_BACK, mask)
//!   vkCmdSetStencilReference  (FRONT_AND_BACK, 1)
//!
//! Same final pixels as Rung BB: pixel(3,3) blue inside
//! the pass-1 region, pixel(0,0) clear-black outside.
//!
//! Pre-fix, the five entry points were stubs / no-ops
//! (vkCmdSetStencilTestEnable was ext_state_stub_u32 and
//! the other four ignored their parameters); the pipeline's
//! disabled stencil leaked through and pass 2's blue
//! painted the whole framebuffer.
//!
...
fn main() -> std::process::ExitCode {
    ...
    // Pipeline declares stencil test OFF + default
    // (no-op) face state.  Every effective stencil field
    // comes from dynamic-state setters below.  Both
    // pipelines share the same depth-stencil block.
    let default_face = vk::StencilOpState {
    ...
    let ds_off = vk::PipelineDepthStencilStateCreateInfo::default()
    ...
    unsafe {
        ...
        let both = vk::StencilFaceFlags::FRONT_AND_BACK;

        // ── Pass 1: turn stencil on + REPLACE/ALWAYS ref=1.
        device.cmd_set_stencil_test_enable(cb, true);
        device.cmd_set_stencil_op(cb, both,
            vk::StencilOp::KEEP,    // fail
            vk::StencilOp::REPLACE, // pass
            vk::StencilOp::KEEP,    // depth_fail
            vk::CompareOp::ALWAYS);
        device.cmd_set_stencil_compare_mask(cb, both, 0xff);
        device.cmd_set_stencil_write_mask  (cb, both, 0xff);
        device.cmd_set_stencil_reference   (cb, both, 1);
        ...
        // ── Pass 2: flip to EQUAL/KEEP read-only.  Test
        // stays on; compare_mask + reference unchanged
        // (they're still 0xff + 1 from above).
        device.cmd_set_stencil_op(cb, both,
            vk::StencilOp::KEEP,
            vk::StencilOp::KEEP,
            vk::StencilOp::KEEP,
            vk::CompareOp::EQUAL);
        device.cmd_set_stencil_write_mask(cb, both, 0x00);
        device.cmd_bind_pipeline(cb, vk::PipelineBindPoint::GRAPHICS,
    ...
    if ok {
        println!("PASS: dynamic stencil state drove the two-pass mask correctly");
        0.into()
    } else {
        eprintln!("FAIL: dynamic stencil state ignored -- pipeline-disabled stencil leaked");
        1.into()
    }
}
