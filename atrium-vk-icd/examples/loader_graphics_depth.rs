//! `examples/loader_graphics_depth` — depth test through the
//! Khronos loader.  Renders two overlapping triangles at
//! distinct NDC z values with depth-test enabled, asserts the
//! nearer triangle wins per-pixel.
//!
//! Scene:
//!
...
fn main() -> std::process::ExitCode {
    ...
    // FS green colour (0.2, 1.0, 0.2, 1.0) -> u8 quantised
    // (51, 255, 51, 255).  Front wins, back rejected.
    let i = (3 * W as usize + 3) * 4;
    let got = [pixels[i], pixels[i+1], pixels[i+2], pixels[i+3]];
    let want_green = [51u8, 255, 51, 255];
    let red        = [255u8, 51, 51, 255];
    println!("pixel(3,3) = {:?}  (want green {:?}, NOT red {:?})",
        got, want_green, red);
    let ok = got == want_green;
    ...
    if ok {
        println!("PASS: depth test rejected the back triangle at overlapping pixels");
        0.into()
    } else {
        eprintln!("FAIL: pixel(3,3) didn't match the front-triangle colour");
        1.into()
    }
}
