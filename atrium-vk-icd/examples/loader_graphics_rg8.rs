...
//! [RG8 variant] two-channel texture, UVs interpolated.
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = sm as *mut u8;
        // 2x2 R8G8_UNORM texture: TWO bytes per texel (8 bytes
        // total), stored natively at 2 bytes/texel with a
        // width*2 row stride.  Each texel has a high R and a low
        // G so R != G proves both channels are read from the
        // right offsets.  The sampler returns (R, G, 0, 1).
        let texels: [[u8; 2]; 4] = [
    ...
    // RG8 sampling yields (R, G, 0, 1).  All texels are
    // (R=200, G=40), so the sample is ~ (200, 40, 0, 255): R high,
    // G low-but-nonzero, B forced to zero, A to one.  A wrong
    // 2-byte stride / channel offset would not give R>>G>0,B=0.
    let ok = r > 150 && g > 10 && g < 90 && b == 0 && a == 255;
    ...
    if ok {
        println!("PASS: RG8 two-channel texture sampled natively -> (R,G,0,1)");
        0.into()
    } else {
        eprintln!("FAIL: RG8 sample not (R,G,0,1) (R={r} G={g} B={b} A={a})");
        1.into()
    }
}
