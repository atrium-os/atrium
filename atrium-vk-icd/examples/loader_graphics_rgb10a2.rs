//! `examples/loader_graphics_rgb10a2` — packed
//! `VK_FORMAT_A2B10G10R10_UNORM_PACK32` vertex attribute
//! (a 32-bit word: 10-bit R/G/B + 2-bit A).  Proves the
//! daemon's `assemble_vertices` unpacks the bitfields and
//! normalises (R/G/B by 1023, A by 3) before handing the
//! stream to the VS.
//!
//! All three vertices share a packed colour decoding to
//! (1023/1023, 512/1023, 0, 3/3) ≈ (1.0, 0.5005, 0.0, 1.0).
//! Vertex layout: 12 B vec3 position + 4 B packed colour =
//! 16 B stride.  Discriminator: pixel(4,4) ≈ (255, 128, 0,
//! 255) -- the green lane (512/1023 ≈ 0.5005 -> 128) proves
//! the 10-bit field extraction + normalise, not a byte copy.
...
/// Pack (r,g,b,a) 10/10/10/2-bit fields into an
/// A2B10G10R10_UNORM_PACK32 word: R[0..10] G[10..20]
/// B[20..30] A[30..32].
fn pack_a2b10g10r10(r: u32, g: u32, b: u32, a: u32) -> u32 {
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = map as *mut u8;
        // All three vertices share a packed colour decoding
        // to R=1023/1023=1.0, G=512/1023≈0.5005, B=0, A=3/3=1.
        // Stride = 12 (pos) + 4 (packed u32) = 16.
        let packed = pack_a2b10g10r10(1023, 512, 0, 3);
        let verts: [[f32; 3]; 3] = [
            [-0.5, -0.5, 0.0],
            [ 0.5, -0.5, 0.0],
            [ 0.0,  0.5, 0.0],
        ...
        for (i, pos) in verts.iter().enumerate() {
            ...
            for k in 0..4 { *p.add(base + 12 + k) = pb[k]; }
        }
    }
    ...
    let attrs = [
        ...
        vk::VertexInputAttributeDescription {
            ...
            format: vk::Format::A2B10G10R10_UNORM_PACK32, offset: 12,
        },
    ];
    ...
    let b = pixels[i + 2];
    let a = pixels[i + 3];
    println!("pixel(4,4) = ({r},{g},{b},{a})  (want ~255,128,0,255 from A2B10G10R10)");
    // Packed (1023,512,0,3) -> (1.0, 0.5005, 0.0, 1.0).
    // Green 512/1023 ≈ 0.5005 -> 128 (allow 127/128) proves
    // the 10-bit field extraction + normalise; R=255 / B=0 /
    // A=255 confirm the other lanes.
    ...
    if ok {
        println!("PASS: A2B10G10R10_UNORM_PACK32 vertex colour unpacked + normalised");
        0.into()
    } else {
        eprintln!("FAIL: RGB10A2 vertex colour not decoded correctly ({r},{g},{b},{a})");
        1.into()
    }
}
