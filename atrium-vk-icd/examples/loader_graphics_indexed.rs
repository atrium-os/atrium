//! `examples/loader_graphics_indexed` — like
//! `loader_graphics_roundtrip` but the triangle is fed through
//! a vertex buffer + index buffer + `vkCmdDrawIndexed` instead
//! of a plain `vkCmdDraw`.  Proves the indexed-draw wire works
//! end-to-end:
//!
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = imap as *mut u32;
        // Two triangles, both CCW in NDC:
        //   T0 = (0, 1, 2)  bottom-left, bottom-right, top-right
        //   T1 = (0, 2, 3)  bottom-left, top-right, top-left
        for (k, v) in [0u32, 1, 2, 0, 2, 3].iter().enumerate() {
    ...
    unsafe {
        ...
        device.cmd_draw_indexed(cb, 6, 1, 0, 0, 0);
    ...
    let p33 = px(3, 3);
    let p55 = px(5, 5);
    println!("pixel(3,3) = {:?} (want {:?})", p33, red);
    println!("pixel(5,5) = {:?} (want {:?})", p55, red);
    // Two-triangle quad covers (~1..6) in both axes; (3,3)
    // sits in the first indexed triangle, (5,5) in the
    // second.  Both must hit the FS colour for the indexed
    // draw to be considered working -- a one-triangle test
    // would let `index_count=6` with broken second-tri
    // dispatch slip through.
    let ok = p33 == red && p55 == red;
    ...
    if ok {
        println!("PASS: both indexed triangles rendered + pixels reached the client");
        0.into()
    } else {
        eprintln!("FAIL: indexed-draw pixels don't match the FS colour");
        1.into()
    }
}
