//! `examples/loader_graphics_strip` — proves
//! `VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP` is honoured.
//! Pipeline declares TriangleStrip; the app submits a
//! 4-vertex fullscreen-quad buffer.  Under strip rules,
//! these 4 vertices form 2 triangles (a quad).  Under the
//! legacy TriangleList interpretation the daemon would see
//! one triangle plus a leftover vertex and only paint
//! roughly the lower-left half.
//!
//! Pre-fix, `VkPipelineInputAssemblyStateCreateInfo` wasn't
//! parsed and the rasterizer hardcoded
//! `tri_count = vertex_count / 3`; strip apps got the
//! wrong primitives.
//!
...
fn main() -> std::process::ExitCode {
    ...
    // 4 × vec3 = 48 B (kept identical to the round-trip
    // rung -- size carries enough alignment headroom even
    // though strip needs strictly only 48 B here).
    ...
    unsafe {
        ...
        // Triangle-strip quad covering NDC [-1, 1]^2.
        // Vertices in strip-order (TL, TR, BL, BR):
        //   v0 (-1,-1) v1 (1,-1)
        //   v2 (-1, 1) v3 (1, 1)
        // Triangle 0 (even t=0): v0, v1, v2 → top half.
        // Triangle 1 (odd  t=1): swapped to v2, v1, v3
        //   (Vulkan strip rule) → bottom half.
        // Together: full-quad coverage.
        let verts: [[f32; 3]; 4] = [
            [-1.0, -1.0, 0.0],
            [ 1.0, -1.0, 0.0],
            [-1.0,  1.0, 0.0],
    ...
    unsafe {
        ...
        device.cmd_draw(cb, 4, 1, 0, 0);  // 4 verts -> 2 strip triangles -> full quad
    ...
    // Quad covers the entire NDC viewport.  Check all four
    // corners + the centre -- any corner clear-black would
    // mean only one strip triangle painted.
    let corners = [px(0, 0), px(7, 0), px(0, 7), px(7, 7), px(3, 3)];
    println!("corners + centre = {:?}  (want all {:?})", corners, red);
    let ok = corners.iter().all(|c| *c == red);
    ...
    if ok {
        println!("PASS: TRIANGLE_STRIP quad covered all 4 corners + centre");
        0.into()
    } else {
        eprintln!("FAIL: strip rasterized as one triangle (TriangleList path leaked?)");
        1.into()
    }
}
