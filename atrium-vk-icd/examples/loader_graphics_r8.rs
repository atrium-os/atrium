...
//! [R8 variant] single-channel texture, UVs interpolated.
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = sm as *mut u8;
        // 2x2 R8_UNORM texture: ONE byte per texel (4 bytes
        // total), stored natively -- the daemon allocates the
        // image at 1 byte/texel and the sampler reads with a
        // width*1 row stride.  Distinct values per texel so a
        // wrong (width*4) stride would mis-sample.  The sampler
        // returns (R, 0, 0, 1) for an R8 texture.
        let texels: [u8; 4] = [64, 128, 192, 255];
    ...
    // R8 sampling yields (R, 0, 0, 1): the red channel carries
    // the (linearly blended) texel value, G and B are forced to
    // zero, A to one.  R lands somewhere in the 64..255 spread of
    // the four texels.  A daemon that mis-stored / mis-strided
    // the 1-byte texels would not produce a clean (R, 0, 0, 1).
    let ok = r > 64 && r < 255 && g == 0 && b == 0 && a == 255;
    ...
    if ok {
        println!("PASS: R8 single-channel texture sampled natively -> (R,0,0,1)");
        0.into()
    } else {
        eprintln!("FAIL: R8 sample not (R,0,0,1) (R={r} G={g} B={b} A={a})");
        1.into()
    }
}
