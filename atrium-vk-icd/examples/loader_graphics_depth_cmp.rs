//! `examples/loader_graphics_depth_cmp` — proves the
//! `vkCmdSetDepthCompareOp` dynamic state actually
//! overrides the pipeline's hardcoded `compareOp = LESS`.
//! Same two-triangle scene as Rung L; depth test is ON.
//! Mid-cmdbuf the app calls
//! `vkCmdSetDepthCompareOp(VK_COMPARE_OP_NEVER)` -- every
//! depth test must now FAIL, so neither triangle should
//! produce colour output.  Pixel(3,3) ends up clear-black.
//!
//! Pre-fix, vkCmdSetDepthCompareOp was an
//! `ext_state_stub_u32!` no-op AND the rasterizer
//! hardcoded LESS regardless of pipeline state -- so the
//! front triangle (z=0.1) would always pass and pixel
//! ended up green.
//!
//! Scene:
//!
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        ...
        // ── Dynamic override: depth test becomes NEVER.
        // No fragment can pass; both triangles get rejected.
        device.cmd_set_depth_compare_op(cb, vk::CompareOp::NEVER);
    ...
    // With compareOp=NEVER the depth test must fail for
    // every fragment; both draws produce no colour and the
    // pixel stays at the clear value.
    ...
    if ok {
        println!("PASS: vkCmdSetDepthCompareOp(NEVER) rejected every fragment");
        0.into()
    } else {
        eprintln!("FAIL: dynamic compare op ignored -- LESS leaked, front painted green");
        1.into()
    }
}
