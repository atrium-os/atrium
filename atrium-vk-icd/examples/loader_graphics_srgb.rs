...
//! [sRGB variant] UVs interpolated across the triangle.
...
fn build_textured_fs() -> Vec<u8> {
    ...
    let color = b.image_sample_implicit_lod(vec4, None, sampler_val, uv, None, vec![]).unwrap();
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = sm as *mut u8;
        // 2x2 texture in R8G8B8A8_SRGB; all four texels are a
        // mid-grey sRGB value 188/255 = 0.737.  The sRGB EOTF
        // maps that to linear ((0.737+0.055)/1.055)^2.4 ~= 0.503,
        // i.e. ~128 when written to the linear RGBA8 target.  A
        // daemon that ignored the sRGB format would pass 188
        // straight through.
        let texels: [[u8; 4]; 4] = [
            [188, 188, 188, 255],
            [188, 188, 188, 255],
    ...
    // The texels are sRGB 188; after the EOTF the sampled
    // linear value is ~0.503 -> ~128 in the target.  Assert all
    // three colour channels landed near 128 (well below the
    // stored 188), proving the sRGB->linear decode ran; a
    // format-blind daemon would leave them at ~188.  Grey, so
    // R~=G~=B.
    let near_128 = |v: u8| v >= 100 && v <= 156;
    let ok = near_128(r) && near_128(g) && near_128(b) && a == 255;
    ...
    if ok {
        println!("PASS: sRGB texture decoded via EOTF (188 -> ~128 linear)");
        0.into()
    } else {
        eprintln!("FAIL: sRGB not decoded (R={r} G={g} B={b} A={a}); expected ~128, stored 188");
        1.into()
    }
}
