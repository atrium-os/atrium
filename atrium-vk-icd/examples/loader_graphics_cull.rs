//! `examples/loader_graphics_cull` — proves the pipeline's
//! `cullMode` + `frontFace` are extracted from the create
//! info and honoured by the rasterizer.  The triangle's
//! vertex order makes it CW in screen-space; pipeline says
//! `cullMode = BACK` + `frontFace = COUNTER_CLOCKWISE`,
//! which means CW triangles are back-facing and should be
//! discarded.  Pixel(3,3) ends up clear-black (not red).
//!
...
fn main() -> std::process::ExitCode {
    ...
    let center = px(3, 3);
    println!("pixel(3,3) after cull -> {:?}  (want clear {:?}, NOT red {:?})",
        center, clear, red);
    let ok = center == clear;
    ...
    if ok {
        println!("PASS: back-facing (CW) triangle culled; pixel stayed clear");
        0.into()
    } else {
        eprintln!("FAIL: back-facing triangle leaked through cull -- pipeline cull state ignored?");
        1.into()
    }
}
