//! `examples/loader_graphics_discard` — proves
//! `vkCmdSetRasterizerDiscardEnable(true)` actually
//! short-circuits the dispatch.  Pipeline declares no
//! discard (default); mid-cmdbuf the app sets the dynamic
//! discard on -- the rasterizer must be bypassed entirely.
//! Pixel(3,3) ends up clear-black instead of the FS
//! colour.
//!
//! Pre-fix, vkCmdSetRasterizerDiscardEnable was an
//! `ext_state_stub_u32!` no-op AND the pipeline's
//! `rasterizerDiscardEnable` field wasn't read from the
//! create info; the daemon always rasterized.
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        ...
        // ── Dynamic override: turn on rasterizer discard.
        // The draw call below must produce no fragments.
        device.cmd_set_rasterizer_discard_enable(cb, true);
    ...
    let clear  = [0u8, 0, 0, 0];
    let center = px(3, 3);
    println!("pixel(3,3) after discard -> {:?}  (want clear {:?}, NOT red {:?})",
        center, clear, red);
    let ok = center == clear;
    ...
    if ok {
        println!("PASS: vkCmdSetRasterizerDiscardEnable(true) short-circuited the draw");
        0.into()
    } else {
        eprintln!("FAIL: rasterizer discard ignored -- triangle painted through");
        1.into()
    }
}
