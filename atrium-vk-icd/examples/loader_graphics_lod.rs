//! `examples/loader_graphics_lod` — implicit-LOD mip
//! selection from screen-space derivatives.  Uploads a
//! 2-level mip chain (level 0 = 2×2 red, level 1 = 1×1
//! blue) and samples with `OpImageSampleImplicitLod` (no
//! explicit LOD).  The triangle maps a wide UV range
//! (0..8) over a small (~4px) region, so the texture is
//! heavily minified.  The rasterizer finite-differences the
//! perspective-correct UV varying across the pixel quad,
//! computes LOD = log2(footprint-in-texels), and redirects
//! the descriptor to the selected mip -> level 1 (blue).
//!
//! Discriminator: pixel(4, 4) = blue (0, 0, 255, 255).
//! With UV 0..1 (magnification) LOD would be <= 0 -> mip 0
//! (red); blue proves the rasterizer computed a real
//! minification LOD and picked the coarse mip implicitly.
//!
...
fn build_textured_fs() -> Vec<u8> {
    ...
    // `OpImageSampleImplicitLod`: NO explicit LOD.  The
    // rasterizer computes the mip from the screen-space UV
    // gradient (the texture is minified here) and redirects
    // the descriptor to that mip level.
    let color = b.image_sample_implicit_lod(
        vec4, None, sampler_val, uv, None, vec![]).unwrap();
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        ...
        //   0..12  position (vec3)
        //   12..20 UV (vec2)
        // Wide UV range (0..8) over a small (~4px) triangle
        // -> large screen-space UV gradient -> the texture is
        // heavily minified, so implicit LOD selects the
        // coarsest mip (level 1 = blue).  With UV 0..1 it
        // would be magnified (mip 0 = red).
    ...
    if ok {
        println!("PASS: implicit LOD picked mip 1 (blue) from the minified UV gradient");
        0.into()
    } else {
        eprintln!("FAIL: pixel(4,4) not blue -- implicit LOD didn't select the coarse mip");
        1.into()
    }
}
