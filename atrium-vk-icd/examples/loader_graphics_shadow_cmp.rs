//! `examples/loader_graphics_shadow_cmp` — sampler
//! `compareOp` (other than the legacy LEQUAL) honoured.
//!
//! The sampler sets `compareEnable = true` +
//! `compareOp = VK_COMPARE_OP_LESS`.  Stored depth R = 0.5,
//! dref = 0.25.  Vulkan's rule is `dref compareOp texel`,
//! so LESS gives `0.25 < 0.5 = true -> 1.0 (lit)`.
//! pixel(4,4) = red (255,0,0,255).
//!
//! Discriminator: the legacy frontend-synthesised path
//! (`texel <= dref` = `0.5 <= 0.25 = false -> 0.0`) would
//! produce BLACK.  Red vs. black distinguishes "sampler
//! compareOp honoured" from "hardwired LEQUAL".  This only
//! works because the runtime Dref helper reads the
//! sampler's compare_enable/compare_op (sampler state the
//! shader compiler can't see).
//!
...
fn build_textured_fs() -> Vec<u8> {
    ...
    // Dref (depth reference) = 0.75.  Stored texture R = 0.5
    // (128/255).  The frontend lowers OpImageSampleDref* to
    // `r <= dref ? 1.0 : 0.0`; 0.5 <= 0.75 -> 1.0 (lit).
    let c_dref = b.constant_bit32(f32_ty, 0.25f32.to_bits());
    ...
    // OpImageSampleDrefImplicitLod returns a scalar f32 (the
    // comparison result).  Output (result, 0, 0, 1) so a lit
    // sample reads back as pure red.
    let cmp = b.image_sample_dref_implicit_lod(
        f32_ty, None, sampler_val, uv, c_dref, None, vec![]).unwrap();
    let color = b.composite_construct(vec4, None, vec![cmp, c_zero, c_zero, c_one]).unwrap();
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = sm as *mut u8;
        // 2x2 depth texture: every texel stores R = 128
        // (= 0.5 in UNORM), the "scene depth" the shadow
        // compare reads.  G/B unused, A = 255.
        let texels: [[u8; 4]; 4] = [
            [128, 0, 0, 255],
            [128, 0, 0, 255],
            [128, 0, 0, 255],
    ...
    let b = pixels[i + 2];
    let a = pixels[i + 3];
    println!("pixel(4,4) = ({r},{g},{b},{a})  (want lit red 255,0,0,255 from compareOp=LESS)");
    // compareOp=LESS: dref(0.25) < texel(0.5) -> 1.0 (lit)
    // -> red.  The legacy LEQUAL path (texel<=dref =
    // 0.5<=0.25 = false) would give black, so red proves
    // the sampler's compareOp was honoured.
    let ok = r == 255 && g == 0 && b == 0 && a == 255;
    ...
    if ok {
        println!("PASS: sampler compareOp=LESS honoured (dref 0.25 < texel 0.5 = lit)");
        0.into()
    } else {
        eprintln!("FAIL: pixel(4,4) not lit red -- compareOp=LESS not honoured (LEQUAL leaked?)");
        1.into()
    }
}
