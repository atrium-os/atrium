//! `examples/loader_graphics_trifan` — Rung ZZ: TriangleFan
//! topology (all triangles share vertex 0).
//!
//! Three vertices at distinct clip positions are drawn with
//! `VK_PRIMITIVE_TOPOLOGY_POINT_LIST`.  Each carries a colour
//! varying; the daemon's point path runs the VS per vertex,
//! perspective-divides + viewport-maps it to a window pixel,
//! depth-tests, and shades that single fragment with the
//! vertex's colour.
//!
//!   P0 clip(-0.5,-0.5) -> pixel (2,2) red
//!   P1 clip( 0.0, 0.0) -> pixel (4,4) green
//!   P2 clip( 0.5, 0.5) -> pixel (6,6) blue
//!
//! Discriminator: exactly those three pixels are lit with their
//! colours and the rest of the framebuffer stays clear.  Under
//! the prior behaviour (PointList fell back to TriangleList) the
//! three vertices would have formed one filled triangle instead.
...
/// VS: in vec2 in_pos@0, in vec3 in_color@1 -> gl_Position + v_color@0.
fn build_vs() -> Vec<u8> {
    ...
    b.decorate(in_col, Decoration::Location, vec![rspirv::dr::Operand::LiteralBit32(1)]);
...
fn build_fs() -> Vec<u8> {
    ...
    let ptr_out_vec4 = b.type_pointer(None, StorageClass::Output, vec4);
    let in_col = b.variable(ptr_in_vec3, None, StorageClass::Input, None);
    b.decorate(in_col, Decoration::Location, vec![rspirv::dr::Operand::LiteralBit32(0)]);
...
fn main() -> std::process::ExitCode {
    ...
    // 4 verts: pos(vec2) + color(vec3) = 20 B/v -> 80 B.
    ...
    unsafe {
        ...
        // pos.x, pos.y, r, g, b -- fan: v0 = centre, v1..v3 outer.
        // Triangles (0,1,2) lower wedge + (0,2,3) right wedge.
        // All white.
        let verts: [[f32; 5]; 4] = [
            [ 0.0,  0.0,  1.0, 1.0, 1.0], // v0 centre -> (4,4)
            [-0.8, -0.8,  1.0, 1.0, 1.0], // v1 -> (0.8,0.8)
            [ 0.8, -0.8,  1.0, 1.0, 1.0], // v2 -> (7.2,0.8)
            [ 0.8,  0.8,  1.0, 1.0, 1.0], // v3 -> (7.2,7.2)
        ];
        let mut i = 0;
        for v in verts { for f in v { p.add(i).write_unaligned(f); i += 1; } }
    }
    unsafe { device.unmap_memory(vmem); }
    println!("vbuf write 4 verts (fan)           -> OK");
    ...
    let attrs = [
        ...
        vk::VertexInputAttributeDescription {
            location: 1, binding: 0, format: vk::Format::R32G32B32_SFLOAT, offset: 8,
        },
    ];
    ...
    // (4,1): inside the FIRST fan triangle (0,1,2) lower wedge.
    // (6,4): inside the SECOND fan triangle (0,2,3) right wedge --
    //        only drawn because the fan reuses vertex 0; a
    //        TriangleList of 4 verts makes just one triangle and
    //        drops v3, leaving this pixel clear.
    // (0,7): outside the fan entirely.
    let first_tri = lit(4, 1);
    let second_tri = lit(6, 4);
    let outside = lit(0, 7);
    let total = pixels.chunks_exact(4).filter(|px| (px[0]|px[1]|px[2]) != 0).count();
    println!("first_tri(4,1)={first_tri}  second_tri(6,4)={second_tri}                outside(0,7)={outside}  lit={total}");

    // Both fan triangles drew; the fan covers more than a single
    // triangle would, and the far corner stays clear.
    let ok = first_tri && second_tri && !outside;
    ...
    if ok {
        println!("PASS: TriangleFan rasterized triangles sharing vertex 0");
        0.into()
    } else {
        eprintln!("FAIL: fan not rasterized as expected \
                   (first={first_tri} second={second_tri} outside={outside})");
        1.into()
    }
}
