//! `examples/loader_graphics_linestrip` — Rung YY: LineStrip
//! topology (a connected polyline).
//!
//! A single horizontal segment is drawn with
//! `VK_PRIMITIVE_TOPOLOGY_LINE_LIST` from a red left endpoint to
//! a blue right endpoint, both at clip-y = 0 (framebuffer row
//! 4).  The daemon's line path runs the VS for both endpoints,
//! viewport-maps them, walks the segment by DDA, and shades each
//! fragment with the perspective-correctly interpolated colour.
//!
//!   V0 clip(-0.9, 0) red  -> ~pixel (0,4)
//!   V1 clip( 0.9, 0) blue -> ~pixel (7,4)
//!
//! Discriminator: row 4 has many lit pixels forming the line;
//! the left end is red-dominant, the right end blue-dominant
//! (proof of interpolation along the segment); a pixel off the
//! line stays clear.  Under the prior behaviour (LineList fell
//! back to TriangleList) two vertices can't form a triangle, so
...
fn main() -> std::process::ExitCode {
    ...
    // 2 verts: pos(vec2) + color(vec3) = 20 B/v.
    ...
    unsafe {
        ...
        // pos.x, pos.y, r, g, b -- 3 verts forming an L:
        // seg0 (V0->V1) horizontal at row 6, seg1 (V1->V2)
        // vertical down column 4.  All white.
        let verts: [[f32; 5]; 3] = [
            [-0.9,  0.5,  1.0, 1.0, 1.0], // V0 -> ~(0,6)
            [ 0.0,  0.5,  1.0, 1.0, 1.0], // V1 -> ~(4,6) (shared corner)
            [ 0.0, -0.5,  1.0, 1.0, 1.0], // V2 -> ~(4,2)
        ];
        let mut i = 0;
        for v in verts { for f in v { p.add(i).write_unaligned(f); i += 1; } }
    }
    unsafe { device.unmap_memory(vmem); }
    println!("vbuf write 3 verts (strip)         -> OK");
    ...
    // seg0 (V0->V1): horizontal run along row 6.
    let row6_lit = (0..W as usize).filter(|&x| lit(x, 6)).count();
    // seg1 (V1->V2): vertical run down column 4 -- only drawn if
    // the strip connected the 2nd segment (LineList would drop V2).
    let col4_vert = lit(4, 3) && lit(4, 4) && lit(4, 5);
    // Off-strip pixel must stay clear.
    let off = lit(0, 0);
    println!("row6 lit={row6_lit}  col4 vertical(4,3..5)={col4_vert}  off(0,0)_lit={off}");

    // Both segments of the connected polyline are present.
    let ok = row6_lit >= 4 && col4_vert && !off;
    ...
    if ok {
        println!("PASS: LineStrip rasterized a connected 2-segment polyline");
        0.into()
    } else {
        eprintln!("FAIL: linestrip not connected as expected \
                   (row6_lit={row6_lit} col4_vert={col4_vert} off={off})");
        1.into()
    }
}
