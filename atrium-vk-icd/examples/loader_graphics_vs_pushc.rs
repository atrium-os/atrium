//! `examples/loader_graphics_vs_pushc` — push constants flowing
//! into the VERTEX shader through the Khronos loader.  Rung I
//! exercised FS push constants; this one's the VS-side
//! companion -- proves the same FrameOp::PushConstants buffer
//! reaches `vs_main`'s `pc_ptr` argument, not just `fs_main`'s.
//!
//! Design: the VS reads a `vec4 offset` from a push constant
//! block and adds (offset.xy, 0) to every NDC vertex position
//! before emitting `gl_Position`.  The push is `(0.5, 0.0,
//! 0.0, 0.0)` so the triangle slides right by half NDC width
//! (= 2 pixels on the 8x8 framebuffer at viewport scaling
//! `pixel = (ndc + 1) * 4`).
//!
//! Original NDC triangle vertices:
//!   V0 (-0.5, -0.5)  -> screen (2, 2)
//!   V1 ( 0.5, -0.5)  -> screen (6, 2)
//!   V2 ( 0.0,  0.5)  -> screen (4, 6)
//!
//! After the VS adds +0.5 to x:
//!   V0' ( 0.0, -0.5) -> screen (4, 2)
//!   V1' ( 1.0, -0.5) -> screen (8, 2)  (off the right edge -- clipped)
//!   V2' ( 0.5,  0.5) -> screen (6, 6)
//!
//! Pixel(3,3) -- inside the ORIGINAL triangle -- ends up
//! clear (0,0,0,0) since the shifted triangle no longer
//! covers it.  Pixel(5,3) -- inside the SHIFTED triangle but
//! outside the original -- ends up the FS colour.  If the
//! VS push constant weren't reaching `vs_main`, the offset
//! would be (0,0,0,0) (push_constants buffer reads as zeros
//! from a null/unwired pointer) and the original triangle
//! would render in its original position.
...
/// VS that reads `vec4 offset` from push constant and shifts
/// the input position by `(offset.x, offset.y, 0)` before
/// emitting gl_Position.
fn build_offset_vs() -> Vec<u8> {
    ...
    // PushConstantBlock { vec4 offset; } at offset 0.
    ...
    let c_zero_u = b.constant_bit32(u32_ty, 0u32);
    ...
    // Read push constant offset.xy.
    let pc_src = b.access_chain(ptr_pc_vec4, None, pc_var, vec![c_zero_u]).unwrap();
    let pc4 = b.load(vec4, None, pc_src, None, vec![]).unwrap();
    let ox = b.composite_extract(f32_ty, None, pc4, vec![0]).unwrap();
    let oy = b.composite_extract(f32_ty, None, pc4, vec![1]).unwrap();
    let x2 = b.f_add(f32_ty, None, x, ox).unwrap();
    let y2 = b.f_add(f32_ty, None, y, oy).unwrap();
    let pos4 = b.composite_construct(vec4, None, vec![x2, y2, z, c_one_f]).unwrap();
    let dst = b.access_chain(ptr_out_vec4, None, pv_var, vec![c_zero_i]).unwrap();
    b.store(dst, pos4, None, vec![]).unwrap();
    b.ret().unwrap();
    b.end_function().unwrap();
    b.entry_point(ExecutionModel::Vertex, main, "main",
        vec![in_pos, pv_var, pc_var]);
...
fn main() -> std::process::ExitCode {
    ...
    // Vertex buffer (same triangle as Rung G).
    ...
    // Pipeline layout with VS push-constant range.
    ...
    let clears = [clear];
    // Push offset = (+0.5, 0, 0, 0) into the VS.
    let pushed: [f32; 4] = [0.5, 0.0, 0.0, 0.0];
    let pushed_bytes: Vec<u8> = pushed.iter()
    ...
    unsafe {
        ...
        device.cmd_push_constants(cb, pl,
            vk::ShaderStageFlags::VERTEX, 0, &pushed_bytes);
    ...
    let p33 = px(3, 3);
    let p53 = px(5, 3);
    println!("pixel(3,3) = {:?}  (want clear (0,0,0,0))", p33);
    println!("pixel(5,3) = {:?}  (want red {:?})", p53, red);
    // The shifted triangle no longer covers (3,3) and now
    // covers (5,3).  Both halves must hold:
    //   * (3,3) cleared -> shift actually happened
    //     (otherwise the original triangle would still
    //     paint it red).
    //   * (5,3) red -> the shift landed where expected
    //     (otherwise we wouldn't catch a "PC reaches VS
    //     but with wrong bytes" scenario).
    let ok = p33 == [0u8, 0, 0, 0] && p53 == red;
    ...
    if ok {
        println!("PASS: VS push constant shifted the triangle as expected");
        0.into()
    } else {
        eprintln!("FAIL: triangle didn't shift -- VS isn't reading push constants");
        1.into()
    }
}
