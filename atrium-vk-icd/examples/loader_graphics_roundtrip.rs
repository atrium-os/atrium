//! `examples/loader_graphics_roundtrip` — end-to-end test that
...
fn main() -> std::process::ExitCode {
    ...
    let center = px(3, 3);
    println!("pixel(3,3) after copy+invalidate  -> {:?} (want {:?})", center, red);
    let ok = center == red;
    ...
    if ok {
        println!("PASS: triangle rendered + pixels reached the client");
        0.into()
    } else {
        eprintln!("FAIL: triangle interior pixel doesn't match the FS colour");
        1.into()
    }
}
