//! `examples/loader_graphics_depth_dynamic` — proves the
//! `vkCmdSetDepthTestEnable` dynamic state actually
//! overrides the pipeline's static depth-test policy.
//! Pipeline is created with depth test ENABLED; mid-cmdbuf
//! the app calls `vkCmdSetDepthTestEnable(VK_FALSE)`, so
//! the LESS test should be skipped entirely.  Both triangles
//! paint -- the second-drawn back triangle (red, z=0.5)
//! overwrites the first-drawn front (green, z=0.1) at
//! overlapping pixels.  Pixel(3,3) ends up red.
//!
//! Pre-fix, vkCmdSetDepthTestEnable was an
//! `ext_state_stub_u32!` no-op; the daemon used the
//! pipeline's ENABLED static state and the back triangle
//! got rejected by the LESS test -- pixel stayed green.
//!
//! Scene:
//!
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        ...
        // ── Dynamic override: disable depth test for the
        // entire cmdbuf even though the pipeline turned it
        // on.  Both triangles now paint regardless of NDC.z.
    ...
    // With the dynamic depth test disabled, the LESS test
    // is skipped and the second draw (back, z=0.5, red)
    // overwrites the first (front, z=0.1, green).  We want
    // red.  Green would mean the dynamic override was
    // ignored and the pipeline's ENABLED static test
    // rejected the back triangle.
    ...
    if ok {
        println!("PASS: vkCmdSetDepthTestEnable(false) dynamic override skipped LESS test");
        0.into()
    } else {
        eprintln!("FAIL: pipeline-static depth-test leaked through despite dynamic disable");
        1.into()
    }
}
