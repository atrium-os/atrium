//! `examples/loader_graphics_pcf` — percentage-closer
//! filtering (bilinear shadow comparison).
//!
//! The sampler uses LINEAR filtering + compareEnable +
//! compareOp=LESS, dref = 0.5.  The 2x2 depth texture has
//! its top row at 0.25 (fails `0.5 < 0.25`) and bottom row
//! at 0.75 (passes `0.5 < 0.75`).  The FS samples at the
//! constant texel-grid centre (0.5, 0.5), so PCF compares
//! all 4 taps (results 0, 0, 1, 1) and bilinearly blends
//! them to exactly 0.5.  The FS outputs (frac, frac, frac,
//! 1) -> mid-grey.
//!
//! Discriminator: pixel(4,4) ~ (128,128,128,255).  A point
//! (non-PCF) compare would snap to 0 or 1 (black or white);
//! the intermediate grey proves the 4-tap comparison blend.
//!
//! (Original texture-rung doc retained below for the
//! shared scaffolding.)
//!
...
fn build_textured_fs() -> Vec<u8> {
    ...
    // sampler2DShadow = depth image (Depth flag = 1) +
    // sampler at descriptor set 0, binding 0.
    let img_ty = b.type_image(f32_ty, Dim::Dim2D, /*depth*/1, 0, 0, 1, ImageFormat::Unknown, None);
    ...
    // PCF: dref = 0.5, sampled at the constant texel-grid
    // centre (0.5, 0.5) of a 2x2 depth texture whose top
    // row is 0.25 (fails LESS) and bottom row 0.75 (passes).
    // Bilinear PCF blends the 0/1 tap results to exactly 0.5.
    let c_dref = b.constant_bit32(f32_ty, 0.5f32.to_bits());
    let c_half = b.constant_bit32(f32_ty, 0.5f32.to_bits());
    let c_one  = b.constant_bit32(f32_ty, 1.0f32.to_bits());
    let _ = in_uv; // sample point is constant, not the varying
    let main = b.begin_function(void, None, FunctionControl::NONE, void_fn).unwrap();
    b.begin_block(None).unwrap();
    // Constant uv = (0.5, 0.5) -> exact 2x2 centre.
    let uv = b.constant_composite(vec2, vec![c_half, c_half]);
    let sampler_val = b.load(sampled_ty, None, sampler_var, None, vec![]).unwrap();
    // Dref result is the PCF pass fraction.  Output
    // (frac, frac, frac, 1) -> a grey whose level reports
    // the fraction.
    let cmp = b.image_sample_dref_implicit_lod(
        f32_ty, None, sampler_val, uv, c_dref, None, vec![]).unwrap();
    let color = b.composite_construct(vec4, None, vec![cmp, cmp, cmp, c_one]).unwrap();
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        let p = sm as *mut u8;
        // 2x2 depth texture, row-major.  Top row (y=0) =
        // 0.25 (64) -> fails LESS (dref 0.5 < 0.25 = false).
        // Bottom row (y=1) = 0.75 (191) -> passes
        // (0.5 < 0.75 = true).  PCF at the centre blends
        // 0/0/1/1 -> 0.5.
        let texels: [[u8; 4]; 4] = [
            [64,  0, 0, 255], // (0,0)
            [64,  0, 0, 255], // (1,0)
    ...
    // Upload texture data from staging buffer to image.
    let tex_copy = vk::BufferImageCopy::default()
    ...
    unsafe {
        device.cmd_copy_buffer_to_image(cb, stage, tex_img,
            vk::ImageLayout::TRANSFER_DST_OPTIMAL, &[tex_copy]);
    }
    ...
    let b = pixels[i + 2];
    let a = pixels[i + 3];
    println!("pixel(4,4) = ({r},{g},{b},{a})  (want mid-grey ~128 from PCF fraction 0.5)");
    // PCF blends 4 taps (0,0,1,1) at the centre -> 0.5 ->
    // ~128 grey.  A point compare would give 0 or 255.
    // Allow 127/128 for the 0.5*255 quantisation.
    let grey = |c: u8| c == 127 || c == 128;
    let ok = grey(r) && grey(g) && grey(b) && a == 255;
    ...
    if ok {
        println!("PASS: PCF blended 4 shadow taps to mid-grey (0.5)");
        0.into()
    } else {
        eprintln!("FAIL: pixel(4,4) not mid-grey -- PCF not blending taps (point compare?)");
        1.into()
    }
}
