//! `examples/loader_graphics_cull_dynamic` — proves the
//! `vkCmdSetCullMode` dynamic state actually overrides the
//! pipeline's static cull policy.  Pipeline is created with
//! `cullMode = NONE` (no culling); a CW triangle would
//! normally paint red.  Mid-cmdbuf, `vkCmdSetCullMode(BACK)`
//! plus `vkCmdSetFrontFace(CCW)` should flip the runtime
//! cull state so the same CW triangle is now back-facing
//! and gets discarded.  Pixel(3,3) ends up clear-black.
//!
//! Pre-fix, vkCmdSetCullMode + vkCmdSetFrontFace were
//! `ext_state_stub_u32!` no-ops in the ICD; the daemon
//! never saw the dynamic override and used the pipeline's
//! NONE static cull, so the triangle painted.
//!
...
fn main() -> std::process::ExitCode {
    ...
    unsafe {
        ...
        // Vertex order flipped from the round-trip rung
        // (swap verts[1] and verts[2]).  In Vulkan's Y-down
        // NDC + Y-down framebuffer pixels, this winds CW in
        // screen-space; with default front_face = CCW, the
        // rasterizer flags it as back-facing.
        let verts: [[f32; 3]; 3] = [
            [-0.5, -0.5, 0.0],
            [ 0.0,  0.5, 0.0],
            [ 0.5, -0.5, 0.0],
    ...
    // Rasterization state: cull BACK + front = CCW.  The
    // CW-wound triangle above is back-facing under this
    // policy and must be culled.
    // Pipeline says cullMode=NONE -- without the dynamic
    // override the CW triangle would paint red.
    ...
    unsafe {
        ...
        // ── Dynamic raster override: kick the cull policy
        // away from the pipeline's NONE static state and
        // make the CW triangle back-facing.
        device.cmd_set_cull_mode(cb, vk::CullModeFlags::BACK);
        device.cmd_set_front_face(cb, vk::FrontFace::COUNTER_CLOCKWISE);
    ...
    let center = px(3, 3);
    println!("pixel(3,3) after dyn cull -> {:?}  (want clear {:?}, NOT red {:?})",
        center, clear, red);
    let ok = center == clear;
    ...
    if ok {
        println!("PASS: vkCmdSetCullMode/SetFrontFace override flipped runtime cull policy");
        0.into()
    } else {
        eprintln!("FAIL: dynamic cull state ignored -- pipeline NONE leaked the CW triangle");
        1.into()
    }
}
