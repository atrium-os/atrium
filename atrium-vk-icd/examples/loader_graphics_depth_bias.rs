//! `examples/loader_graphics_depth_bias` — proves the
//! Vulkan polygon-offset (depth bias) path moves a
//! triangle's effective depth.  Front triangle is drawn
//! FIRST at the nearer NDC.z (would normally win the LESS
//! test); applying a large positive depth bias bumps its
//! effective depth past the back triangle, so the back
//! one wins instead.
//!
//! Setup:
//!   Triangle F (drawn 1st) ndc.z = 0.1   green
//!     bias enabled, constant_factor = 4_000_000
//!     -> bias ≈ 4e6 * 2^-23 ≈ 0.477
//!     -> windowed z ≈ 0.1 + 0.477 = 0.577
//!   Triangle B (drawn 2nd) ndc.z = 0.5   red
//!     no bias  -> windowed z = 0.5
//!
//! With the bias honoured:
//!   F writes depth=0.577 first; B's 0.5 LESS than 0.577,
//!   so B's red wins.  pixel(3,3) = red.
//!
//! Without the bias path:
//!   F writes depth=0.1, B's 0.5 fails LESS, F's green
//!   stays.  pixel(3,3) = green.
//!
//! Scene:
//!
//!   Triangle B  (back)   z = 0.5   red    (1.0, 0.2, 0.2)
//!   Triangle F  (front)  z = 0.1   green  (0.2, 1.0, 0.2)
//!
//! Both cover the same screen-space region; pixel(4,4) sits
//! inside both.  Without a depth test the second draw's
//! colour would always win.  With depth-test enabled +
//! `compare_op = LESS` + depth-write on, the front triangle
//! (smaller z) writes to the depth buffer first; the back
//! triangle (larger z) fails the depth comparison at the
//! overlapping pixels and is rejected.  Pixel(4,4) ends up
//! green.
//!
...
fn main() -> std::process::ExitCode {
    ...
    // Pipeline F (drawn first): enable depth bias with a
    // large constant factor so F's effective depth lands
    // past B's.
    let rs_biased = vk::PipelineRasterizationStateCreateInfo::default()
        .polygon_mode(vk::PolygonMode::FILL)
        .cull_mode(vk::CullModeFlags::NONE)
        .front_face(vk::FrontFace::COUNTER_CLOCKWISE)
        .depth_bias_enable(true)
        .depth_bias_constant_factor(4_000_000.0)
        .depth_bias_clamp(0.0)
        .depth_bias_slope_factor(0.0)
        .line_width(1.0);
    let rs_plain = vk::PipelineRasterizationStateCreateInfo::default()
        .polygon_mode(vk::PolygonMode::FILL)
        .cull_mode(vk::CullModeFlags::NONE)
        .front_face(vk::FrontFace::COUNTER_CLOCKWISE)
        .line_width(1.0);
    let make_pipeline = |fs_mod: vk::ShaderModule,
                         rs: &vk::PipelineRasterizationStateCreateInfo<'_>| -> u64 {
    ...
    let pipeline_front: u64 = make_pipeline(fs_front, &rs_biased);
    let pipeline_back: u64  = make_pipeline(fs_back,  &rs_plain);
    ...
    unsafe {
        ...
        // Draw FRONT triangle (z=0.1, green) first -- writes depth.
        device.cmd_bind_pipeline(cb, vk::PipelineBindPoint::GRAPHICS,
            vk::Pipeline::from_raw(pipeline_front));
        device.cmd_draw(cb, 3, 1, 0, 0);
        // Draw BACK triangle (z=0.5, red) second -- should fail
        // the depth test (front already wrote z=0.1 < 0.5) and
        // get rejected at overlapping pixels.
        device.cmd_bind_pipeline(cb, vk::PipelineBindPoint::GRAPHICS,
    ...
    // With depth bias honoured, F's effective depth (~0.58)
    // is past B's (0.5), so B wins the LESS test and red
    // paints over green at the overlapping pixels.
    ...
    if ok {
        println!("PASS: depth bias pushed front triangle past back; back wins the LESS test");
        0.into()
    } else {
        eprintln!("FAIL: depth bias not applied -- front still wins despite bias");
        1.into()
    }
}
