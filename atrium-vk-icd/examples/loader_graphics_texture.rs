...
fn main() -> std::process::ExitCode {
    ...
    // pixel(4,4) sits inside the triangle and -- thanks to
    // barycentric UV interp + LINEAR filtering -- pulls
    // contributions from all four texture corners.
    //   - R nonzero (red and white corners contribute)
    //   - G nonzero (green and white corners contribute)
    //   - B nonzero (blue and white corners contribute)
    //   - A == 255 (every texel has A=255)
    let ok = r > 0 && g > 0 && b > 0 && a == 255;
    ...
    if ok {
        println!("PASS: textured triangle sampled all 2x2 texel corners");
        0.into()
    } else {
        eprintln!("FAIL: pixel(4,4) doesn't look like a textured sample");
        1.into()
    }
}
