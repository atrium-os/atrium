//! `examples/loader_graphics_pushc` — like
//! `loader_graphics_roundtrip` but the fragment shader reads
//! its output colour from a 16-byte push constant instead of
//! a constant in the SPIR-V.  Proves push-constant flow into
//! the FS through the Khronos loader, end-to-end:
//!
//!   loader -> vkCmdPushConstants -> ICD push-constant
//!   FrameOp -> daemon -> fill_image_triangle's pc_ptr -> FS
//!   OpLoad PushConstant -> pixel write -> CopyImgToBuf ->
//!   client invalidate.
//!
//! Pipeline shape mirrors loader_graphics_roundtrip; only
//! diff: pipeline layout has a 16-byte FS push-constant
//! range, the cmd buffer calls `vkCmdPushConstants` with
//! `(0.0, 0.5, 0.75, 1.0)` before draw, and the FS does
//! `OpLoad %vec4 (PushConstantBlock.color)` instead of an
//! inline `OpConstantComposite`.  Asserts pixel(3,3) ==
//! (0, 128, 191, 255) -- the rasterizer's
//! `f32 * 255.0 + 0.5 as u8` quantisation of the pushed
//! values.
...
/// Passthrough VS (vec3 position -> vec4 gl_Position).  Same
/// shape as loader_graphics_roundtrip's VS; the example is
/// self-contained.
...
/// FS: reads `vec4 color` from the 16-byte
/// `PushConstantBlock` at offset 0, writes to Location 0.
/// SPIR-V shape matches the tier2_render_pixels suite's
/// `build_pc_color_fs`.
fn build_pc_color_fs() -> Vec<u8> {
    ...
    // PushConstantBlock { vec4 color; } at offset 0.
...
fn main() -> std::process::ExitCode {
    ...
    // ── Pipeline layout: 16-byte FS push-constant range. ──
    ...
    let clears = [clear];
    // The colour we push.  Rasterizer quantises to
    // `f32 * 255.0 + 0.5 as u8` -> (0, 128, 191, 255).
    ...
    unsafe {
        ...
        device.cmd_push_constants(cb, pl,
            vk::ShaderStageFlags::FRAGMENT, 0, &pushed_bytes);
    ...
    if ok {
        println!("PASS: FS push-constant colour reached the client unchanged");
        0.into()
    } else {
        eprintln!("FAIL: pixel(3,3) doesn't match the pushed colour");
        1.into()
    }
}
