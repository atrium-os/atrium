//! `examples/loader_graphics_half` — half-float (f16)
//! vertex attribute.  The per-vertex colour attribute is
//! `VK_FORMAT_R16G16B16A16_SFLOAT` (8 bytes, 4 × f16 in
//! source).  Proves the daemon's `assemble_vertices`
//! decodes f16 -> f32 before handing the stream to the VS.
//!
//! All three vertices share colour (1.0, 0.5, 0.0, 1.0), so
//! the interpolated fragment is exactly that.  Vertex
//! layout: 12 B vec3 position + 8 B 4×f16 colour = 20 B
//! stride.  Discriminator: pixel(4,4) = (255, 128, 0, 255)
//! -- the 0.5 green lane proves the f16 mantissa/exponent
//! decode (0.5 = 0x3800), not a raw byte copy.
...
/// Encode an f32 to IEEE-754 half (binary16).  Round-to-
/// nearest-even; the test values (1.0, 0.5, 0.0) are exact.
fn f32_to_half(f: f32) -> u16 {
    ...
    if exp <= 0 {
        sign // flush tiny/zero to signed zero (sufficient here)
    } else if exp >= 0x1f {
        sign | 0x7c00 // inf
    } else {
...
/// VS: in_pos vec3 (Loc 0), in_color vec4 (Loc 1).  Outputs
/// gl_Position + Location=0 vec4 varying.
fn build_pos_unorm_color_vs() -> Vec<u8> {
    ...
    let ptr_in_vec4  = b.type_pointer(None, StorageClass::Input, vec4);

    let in_pos = b.variable(ptr_in_vec3, None, StorageClass::Input, None);
    b.decorate(in_pos, Decoration::Location,
        vec![rspirv::dr::Operand::LiteralBit32(0)]);
    let in_col = b.variable(ptr_in_vec4, None, StorageClass::Input, None);
    ...
    let col = b.load(vec4, None, in_col, None, vec![]).unwrap();
...
/// FS: passthrough of vec4 varying to out_color.
fn build_passthrough_color_fs() -> Vec<u8> {
    ...
    b.begin_block(None).unwrap();
    let col = b.load(vec4, None, in_col, None, vec![]).unwrap();
    b.store(out, col, None, vec![]).unwrap();
    b.ret().unwrap();
    b.end_function().unwrap();
...
fn main() -> std::process::ExitCode {
    ...
    let vs_spv = build_pos_unorm_color_vs();
    ...
    // ── Vertex buffer: pos(vec3) + colour(R8G8B8A8_UNORM) per vert ──
    // Layout per vertex (16 B, stride 16):
    //   bytes  0..12  position (3 × f32)
    //   bytes 12..16  colour   (4 × u8, RGBA8 UNORM)
    // Source stays a tight 4-byte colour; the daemon expands
    // it to 4 × f32 in the assembled stream.
    ...
    unsafe {
        let p = map as *mut u8;
        // All three vertices share the same half-float colour
        // (1.0, 0.5, 0.0, 1.0) so the interpolated fragment
        // colour is exactly that -- a clean f16-decode check.
        // Stride = 12 (pos) + 8 (4 x f16) = 20.
        let col: [f32; 4] = [1.0, 0.5, 0.0, 1.0];
        let verts: [[f32; 3]; 3] = [
            [-0.5, -0.5, 0.0],
            [ 0.5, -0.5, 0.0],
            [ 0.0,  0.5, 0.0],
        ...
        for (i, pos) in verts.iter().enumerate() {
            let base = i * 20;
            for j in 0..3 {
                let f = pos[j].to_le_bytes();
                for k in 0..4 { *p.add(base + j*4 + k) = f[k]; }
            }
            for j in 0..4 {
                let h = f32_to_half(col[j]).to_le_bytes();
                for k in 0..2 { *p.add(base + 12 + j*2 + k) = h[k]; }
            }
        }
    }
    ...
    // One binding, stride 16 (12 B pos + 4 B colour).
    ...
    let attrs = [
        ...
        vk::VertexInputAttributeDescription {
            ...
            format: vk::Format::R16G16B16A16_SFLOAT, offset: 12,
        },
    ];
    ...
    let b = pixels[i + 2];
    let a = pixels[i + 3];
    println!("pixel(4,4) = ({r},{g},{b},{a})  (want 255,128,0,255 from f16 (1.0,0.5,0.0,1.0))");
    // All verts share colour (1.0,0.5,0.0,1.0) -> interp is
    // exact.  The 0.5 green lane (-> 128, allow 127) proves
    // the f16 decode; R=255 / B=0 confirm the 1.0/0.0 lanes.
    let g_ok = g == 127 || g == 128;
    let ok = r == 255 && g_ok && b == 0 && a == 255;
    ...
    if ok {
        println!("PASS: R16G16B16A16_SFLOAT (half-float) vertex colour decoded");
        0.into()
    } else {
        eprintln!("FAIL: f16 vertex colour not decoded correctly ({r},{g},{b},{a})");
        1.into()
    }
}
