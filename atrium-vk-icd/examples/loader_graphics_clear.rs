//! `examples/loader_graphics_clear` — proves the
//! BeginRenderPass colour clear is actually applied.
//! Clears the framebuffer to blue (0,0,255,255), draws a
//! small red triangle.  A pixel OUTSIDE the triangle must
//! read back the clear colour (blue); a pixel inside reads
//! the FS red.  Pre-fix, tier-2 never applied the colour
//! clear -- it relied on images being zero-allocated, so a
//! non-covered pixel would have been (0,0,0,0) and a
//! non-zero clear colour was silently dropped.
//!
...
fn main() -> std::process::ExitCode {
    ...
    // Clear to opaque blue so a non-covered pixel proves the
    // clear actually ran (vs the image's zero-alloc).
    let clear = vk::ClearValue { color: vk::ClearColorValue { float32: [0.0, 0.0, 1.0, 1.0] } };
    let clears = [clear];
    unsafe {
    ...
    let center = px(3, 3);   // inside the triangle
    let corner = px(0, 0);   // outside -> clear colour
    println!("pixel(3,3) inside  -> {:?} (want red  {:?})", center, red);
    println!("pixel(0,0) cleared -> {:?} (want blue {:?})", corner, blue);
    // corner==blue proves the BeginRenderPass clear ran; if
    // tier-2 skipped the clear (old behaviour) it would be
    // (0,0,0,0) from the zero-allocated image.
    let ok = center == red && corner == blue;
    ...
    if ok {
        println!("PASS: clear colour applied (blue corner) + triangle (red centre)");
        0.into()
    } else {
        eprintln!("FAIL: clear colour not applied or triangle missing");
        1.into()
    }
}
