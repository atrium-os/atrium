...
echo "########## 3/6 META-EXHAUSTION REGRESSION ##########"
...
echo "########## 4/6 NAMESPACE-CHURN REGRESSION ##########"
...
echo "########## 6/6 DEDUP EXISTENCE-ORACLE (§20.1/§20.2) ##########"
# A SECURITY property that decays silently: any optimisation that
# short-circuits a publish on a registry hit re-opens the oracle, and nothing
# else in the suite would notice. Runs both arms so the GLOBAL positive control
# proves the test still measures something.
sh "$BSD/scripts/vm-dedup-oracle-test.sh" 2>&1 | tee "$OUT/dedup.log" \
    | grep -E "ratio=|verdict:|recovered=|^PASS|^FAIL" || rc=1
grep -q "^PASS" "$OUT/dedup.log" || rc=1
...
echo "dedup:  $(grep -E '^PASS|^FAIL' "$OUT/dedup.log" | tail -1)"
