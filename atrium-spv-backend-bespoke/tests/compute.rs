...
/// CS with a dynamic SSBO index: `ssbo.data[gid.x] = gid.x`.
/// Exercises Op::PtrOffsetDynamic through bespoke -- the
/// frontend lowered this to AccessChain(byte_offset=0) +
/// PtrOffsetDynamic(stride=4), and bespoke emits the
/// shift-and-add address arithmetic.
fn build_dynamic_index_ssbo_cs() -> Vec<u8> {
    use rspirv::binary::Assemble;
    use rspirv::spirv::{
        AddressingModel, BuiltIn, Capability, Decoration, ExecutionMode,
        ExecutionModel, FunctionControl, MemoryModel, StorageClass,
...
#[test]
fn bespoke_compiles_dynamic_ssbo_index() {
    let spv = build_dynamic_index_ssbo_cs();
    let module = translate(&spv).expect("frontend");
    let target = if cfg!(target_os = "macos") {
        Target::Aarch64Darwin
    } else {
    ...
    let out = compile_blob(&module, target)
        .expect("bespoke should compile ssbo.data[gid.x] = gid.x");
    assert!(!out.blob.is_empty());
}

/// CS that does `atomicAdd(ssbo.counter, gid_x)` -- the
/// canonical counter / histogram-bucket increment pattern.
/// Discards the returned old value (it's just used for
/// side effect here).
...
#[test]
fn bespoke_compiles_atomic_iadd() {
    ...
    let out = compile_blob(&module, target)
        .expect("bespoke should compile atomicAdd through the load+add+store \
                 lowering on the serial dispatcher");
    assert!(!out.blob.is_empty());
}
