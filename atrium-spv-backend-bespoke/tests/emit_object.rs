...
#[test]
fn nonempty_fragment_rejected_as_unsupported() {
    use atrium_spv_backend_bespoke::BackendError;

    // Build a module whose body has TWO insts (a Store +
    // a Return). The phase-3 skeleton supports only the
    // empty-Return-only case.
    let entry_block = BlockId(0);
    let mut blocks = HashMap::new();
    blocks.insert(entry_block, Block {
        id: entry_block,
