//! ★ REVIEW TOOLING, not the renderer: paints a Scene to pixels, so a person
//! (nsg-raster) or an oracle (the WPT reftest driver) can compare what the
//! NSG says. Rects are filled with source-over blending; glyphs are
//! rasterized UNHINTED from the same canonical font bytes the scene names
//! (web-font condition 2a), at the run's sub-pixel position. Behind the
//! `raster` feature: never in the worker's build.

use crate::{fontset::FontSet, Rect, Scene, PX};

pub struct Canvas {
    pub w: usize, pub h: usize, pub px: Vec<[f32; 4]>,
...
fn invert(m: &crate::Xform) -> Option<[f64; 6]> {
    let k = crate::XF_ONE as f64;
...
pub fn paint(scene: &Scene, fonts: &FontSet, assets: &std::collections::BTreeMap<String, std::path::PathBuf>) -> Canvas {
...
impl Canvas {
    /// 8-bit RGB, row-major — what a PNG writer or a pixel comparison takes.
    pub fn rgb8(&self) -> Vec<u8> {
        self.px.iter().flat_map(|p| [0, 1, 2].map(|i| (p[i].clamp(0.0, 1.0) * 255.0 + 0.5) as u8)).collect()
    }
}
