...
struct AtlasPage {
    ...
    slot_id: u32,
    /// Bumped on every `pixels` mutation so the dispatcher knows to
    /// re-upload before the next render.
    revision: u64,
    /// Last-uploaded revision. If `revision > uploaded_revision`
    /// the atlas needs to be re-published to the slot.
...
/// Pending atlas re-upload generated by a `shape_text_run` call.
/// The dispatcher hands these to `HeadlessRenderer::process_uploads`.
#[derive(Debug)]
...
impl TextEngine {
    ...
    pub fn shape_text_run(
        ...
        let pending = if page.revision > page.uploaded_revision {
            page.uploaded_revision = page.revision;
            Some(PendingAtlasUpload {
