...
/// Register-pressure variant of the heavy loop: **four**
/// cross-coupled f32 accumulators, five loop-carried Phis,
/// ~12 FMul/FAdd per iteration. Mirrors `build_heavy4_spirv`
/// in the bespoke backend's `emit_freebsd_obj` example.
///
/// This shader overflows the bespoke V-register file's
/// caller-saved tier (V16..V31) and forces it into the
/// callee-saved V8..V15 — i.e. it exercises the opt #5
/// FP `stp d`/`ldp d` prologue+epilogue. Keeping it in the
/// strict three-way corpus means the spill-tier codegen is
/// checked bit-exact against the interpreter oracle.
fn build_heavy4_shader() -> Vec<u8> {
    use rspirv::binary::Assemble;
    use rspirv::spirv::{
        AddressingModel, Capability, Decoration, ExecutionMode,
        ExecutionModel, FunctionControl, LoopControl, MemoryModel,
...
#[test]
fn three_way_heavy4() {
    // Four-accumulator register-pressure variant. Overflows
    // the bespoke V-register file's caller-saved tier and
    // forces the opt #5 callee-saved V8..V15 prologue —
    // this test is what proves that spill-tier codegen is
    // bit-exact against the interpreter oracle + Cranelift.
