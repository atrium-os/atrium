//! Real PAM authentication via libpam FFI — the `auth` + `account` facilities only
//! (Atrium does its own session establishment; see `Ostiarius::authenticate`).
//! OpenPAM ABI (FreeBSD base + macOS host). Feature-gated (`--features pam`) so the
//! default build links no libpam.
//!
//! Flow: `pam_start(service, user, &conv)` → `pam_authenticate` (runs the auth
//! stack; `pam_unix` crypt-compares against `master.passwd`, which needs the
//! privilege ostiarius has) → `pam_acct_mgmt` (account valid?) → `pam_end`. The
//! conversation function supplies the password ostiarius received from vestibulum
//! — no tty prompt.
...
/// array + strings, so they must come from libc `calloc`/`strdup` (not Rust's
/// allocator) — that's the contract.
extern "C" fn converse(
    ...
    let password = appdata as *const c_char; // a C string of the password
    for i in 0..n {
        let m = unsafe { *msg.add(i) }; // msg[i]
...
pub fn authenticate(service: &str, user: &str, password: &str) -> Result<(), String> {
    ...
    // appdata = the password; the conv strdup's it during pam_authenticate, so pw
    // must outlive that call (it does — it lives to the end of this fn).
