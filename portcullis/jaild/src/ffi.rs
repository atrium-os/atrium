...
// ====================================================================
// kqueue — the connection multiplexer for server::serve.
// ====================================================================

/// A kqueue(2) watching fds for readability. Level-triggered: a fd with
/// unread bytes (or a pending EOF) is reported on every wait until it is
/// drained or removed. Closing a watched fd removes it implicitly.
pub struct Kqueue {
    fd: libc::c_int,
}
...
impl Kqueue {
    ...
    fn change(&self, fd: i32, flags: u16) -> io::Result<()> {
    ...
    pub fn add_read(&self, fd: i32) -> io::Result<()> {
        self.change(fd, libc::EV_ADD as u16)
    }

    /// Stop reporting `fd`.
    pub fn remove_read(&self, fd: i32) -> io::Result<()> {
    ...
    pub fn wait(&self, block: bool) -> io::Result<Vec<i32>> {
        ...
        Ok(evs[..n as usize].iter().map(|e| e.ident as i32).collect())
    }
}
