...
- **D5 Tier-2 software Vulkan compute path** (2026-05-21): bespoke ARM64 + Cranelift fallback both runtime-correct end-to-end through atrium-vk-icd. Multi-binding SSBO (1–6 bindings), dynamic AccessChain (`ssbo[i]`), all 12 standard SPIR-V atomic ops, 51 GLSL.std.450 math functions (full f32 transcendentals: sin/cos/tan with mod-π range reduction; exp/exp2/log/log2/pow via Taylor + IEEE-754 exponent reconstruction through Op::Bitcast; atan/asin/acos/atan2 with reciprocal range reduction; sinh/cosh/tanh/asinh/acosh/atanh decomposed via exp/log; plus geometric ops length/distance/normalize/reflect/cross; plus signed/unsigned integer min/max/clamp via Op::Select; plus findILsb/findSMsb/findUMsb via Op::Clz + Op::Rbit; plus packHalf2x16/unpackHalf2x16 via ARM FCVT half-precision, bespoke-only), real-workload integration (histogram, per-vertex Lambert lighting, vec4 Reinhard tonemap, normalize+scale chain, parallel sum-reduction) — 68 cross-backend differential tests (including a 32-thread × 100-iteration LSE atomic race-safety test, a workgroup-shared-memory accumulator, a workgroup-shared array, IsNan/IsInf classification, a storage-image write, and 2D + 3D `imageAtomicAdd`) + 22 vk-icd end-to-end tests byte-identical, 0 ignored. Workgroup-parallel dispatcher live; atomics race-safe via ARMv8.1 LSE; `StorageClass::Workgroup` shared memory (scalars, vectors, arrays, matrices, structs) plumbed end-to-end; storage images (`OpImageRead`/`OpImageWrite` on `image2D`) work end-to-end through the daemon, including image + multi-binding-SSBO co-use; atomic storage-image ops via `OpImageTexelPointer` + LSE, including `image3D`. See [spec/tier2-renderer.md § 15](spec/tier2-renderer.md#15-status--next-steps).
