...
- **D5 Tier-2 software Vulkan compute path** (2026-05-21): bespoke ARM64 + Cranelift fallback both runtime-correct end-to-end through atrium-vk-icd. Multi-binding SSBO (1–6 bindings), dynamic AccessChain (`ssbo[i]`), all 12 standard SPIR-V atomic ops, 32 GLSL.std.450 math functions (incl. length, normalize, reflect, cross, smoothstep, fmod, inverseSqrt, sabs, sin/cos/tan with range-reduction modulo π so the full real line is accepted, exp/exp2/log/log2/pow via Taylor + IEEE-754 exponent reconstruction through Op::Bitcast, atan/asin/acos with reciprocal range reduction over the full real line), real-workload integration (histogram, per-vertex Lambert lighting, vec4 Reinhard tonemap, normalize+scale chain, parallel sum-reduction) — 54 cross-backend differential tests + 18 vk-icd end-to-end tests byte-identical, 0 ignored. See [spec/tier2-renderer.md § 15](spec/tier2-renderer.md#15-status--next-steps).
