...
| 2026-05-05 | M11 (potential GPU-side CAS for textures + compute-result caching) added as future evolution. |
| 2026-05-06 | **M3 CLOSED.** Real frescod binary launches end-to-end in QEMU+HVF: opens /dev/atrium-display0 + /dev/atrium-gpu0 via the native atrium-virtio-gpu kmod (no drm-kmod), enumerates connectors (1280x800@60Hz scanout), loads atrium-core SPIR-V bundle through Mesa lavapipe (3 ops: rect/texture/path), accepts client connections on /tmp/frescod.sock, runs the 30-fps frame loop. atrium-clock-socket connects, creates a 480x480 window at the hint-requested initial_position, dispatches its 16 path nodes per frame (12 ticks + 3 hands + 1 hub), first frame rendered cleanly. Both processes settle into steady state (frescod sleeping in nanslp between frame ticks, client blocked in kqread for events). Final kmod fix that closed it: `ce40004` — explicit `virtqueue_enable_intr(ctrl_vq)` after `virtio_setup_intr` (the cv_wait callback path needed an explicit per-VQ interrupt arm; the earlier `virtqueue_poll` design enabled it as a side effect), plus bounded `cv_timedwait(5*hz)` in `req_resp` to fail-fast on missed interrupts instead of D-state-hanging the caller. M3 chain — RUNBOOK §4 + Mesa lavapipe (`mesa-devel`) + probe-priority fix (`e210165`) + ABI v2 spec (`1683444`) + this kmod fix (`ce40004`) — collectively complete. |
