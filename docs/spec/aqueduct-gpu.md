...
> **Detailed design: `carillon.md` (Carillon).** The sketch below assumed
> a *polling* ivshmem channel ("doesn't depend on MSI-X (which we already
> poll for via `FRESCO_POLL_HZ`)"). That assumption is **superseded** —
> Carillon is **fully doorbell-driven** (both sides sleep on a doorbell,
> no ring-spin, no control-register poll), the venus replacement that
> lets the FreeBSD VM reach the host `MoltenVkBackend`. Read `carillon.md`
> for the authoritative design; §6.1–6.2 here are kept as the original
