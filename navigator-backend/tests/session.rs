...
use navigator_backend::session::{OpenError, SessionLimits, Sessions};
...
fn a_session_opens_and_reads_its_document() {
    ...
    let id = s.open(&small("https://a.test/")).expect("opens");
    assert_eq!(s.len(), 1);
...
fn opening_past_the_session_limit_is_refused() {
    ...
    s.open(&small("https://a.test/")).expect("1");
    s.open(&small("https://b.test/")).expect("2");
    match s.open(&small("https://c.test/")).expect_err("must be refused") {
...
fn closing_a_session_makes_room_for_another() {
    ...
    let mut s = Sessions::new(limits);
    let a = s.open(&small("https://a.test/")).expect("1");
    assert!(s.open(&small("https://b.test/")).is_err());
    assert!(s.close(a));
    s.open(&small("https://b.test/")).expect("room was made");
    assert_eq!(s.len(), 1);
}
...
fn a_document_too_large_for_the_budget_is_refused_by_memory_not_by_count() {
    ...
    match s.open(&big).expect_err("must be refused") {
...
fn an_out_of_profile_document_is_refused_as_that_not_as_memory() {
    ...
    match s.open(&deep).expect_err("must be refused") {
...
fn sessions_share_one_budget() {
    ...
    let a = s.open(&small("https://a.test/")).expect("1");
    let before = s.bytes();
    let b = s.open(&small("https://b.test/")).expect("2");
...
fn a_growing_session_costs_more() {
    ...
    let id = s.open(&small("https://a.test/")).expect("opens");
    let before = s.bytes();
    let sess = s.get_mut(id).expect("is there");
    sess.go(&Transition {
        trigger: "#t".into(), event: "click".into(), anchored: true,
...
fn the_caller_chooses_what_to_evict() {
    ...
    let a = s.open(&small("https://a.test/")).expect("1");
    let b = s.open(&small("https://b.test/")).expect("2");
    let c = s.open(&small("https://c.test/")).expect("3");
...
fn the_default_bound_admits_a_realistic_set_of_sessions() {
    ...
    for i in 0..SessionLimits::default().max_sessions {
        s.open(&small(&format!("https://{i}.test/")))
