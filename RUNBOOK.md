...
**Module-ownership rule (and the audit that enforced it):** a *device-global* resource or action (one that spans GFX + DCN, or outlives any one IP block) belongs to the **base**; a per-block one belongs to its IP module. Two defects of this class were found and fixed — the IH ring/ISR and the device FLR (both were gpu-owned, both could disturb the display behind its back). Register namespaces are otherwise cleanly separated (gpu never touches `regDISP_*` and vice-versa). Two *gaps* remain flagged, not yet code: (1) **display energy isn't federated** — only the gpu registers an energy member (`"gpu0"`); per the display-architecture doc the display should register its own (refresh/PSR/VRR counters). (2) **the VRAM bump allocator (`vram_next`) is gpu-owned** — fine while the gpu is the only VRAM consumer (the display imports BOs by offset), but a second consumer (e.g. a display cursor plane) would want it base-owned.
