...
- **Minimizing the pre-splash flash:** the splash appears once the *kernel* starts, so before it you briefly see (a) the UEFI/TianoCore firmware logo — pre-loader, not removable without reconfiguring EDK2; and (b) the loader's beastie menu + autoboot countdown — suppress with `beastie_disable="YES"` + `autoboot_delay="0"` in `/boot/loader.conf` (the splash preload is independent of the menu). After that the only pre-splash frame is the brief firmware logo.
