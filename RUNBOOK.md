...
- **Reboot reliability:** a guest `reboot` can **hang on shutdown** when the jailed desktop services (jaild/frescod/ostiarius hold procdescs + live jails) don't tear down cleanly, freezing the console and degrading sshd. If that happens, do a clean QEMU restart instead of `kill -9`: `echo quit | nc -U /tmp/qmp.sock` then relaunch `scripts/run-vm.sh …`. (`system_reset` via the monitor also resets the guest but may not re-init the ramfb GOP → black screen; prefer quit+relaunch for a clean firmware boot.) 9p does **not** auto-mount after a fresh boot: `kldload p9fs; mount -t p9fs -o trans=virtio bsd_share /mnt/host`.
  - ⚠️ **`sync` before `quit`, or your `/boot/modules` write is LOST.** `echo quit | nc -U /tmp/qmp.sock` terminates QEMU *immediately* — any guest write still in the buffer cache (a fresh `cp …ko /boot/modules`) never reaches the virtual disk, so the next boot silently loads the **old** module. Symptom that bit us: rebuilt kmods, cp'd them, rebooted, and dmesg showed pre-change behaviour with the *old* `.ko` sizes in `/boot/modules`. Always `sync && sync` in the guest after installing to `/boot/modules` (and ideally re-`ls -la` the sizes) *before* the `quit`.
...
- `base/atrium_gpu_amd_pci.ko` — the newbus `pci` driver: matches `0x1002:0x7550`, enables busmaster, maps BAR5/BAR2 into the **shared softc**, allocates MSI-X, **owns the device-global IH (interrupt-handler) ring + the single ISR** (`ih.c`), and `device_add_child`s two children named after the driver-modules that claim them (`atrium_gpu_amd`, `atrium_gpu_amd_display`). Loaded first. The IH ring is a *device* resource — on real silicon GFX end-of-pipe and DCN vblank ride one ring to one ISR, demuxed by cause — so it lives here, not in the gpu module. The ISR drains the ring under `sc->lock` and dispatches each cookie by cause to `sc->ih_handler[cause]`; each IP child registers its handler via the inline `amd_ih_set_handler` (a function pointer in the shared softc, set under `sc->lock` — no cross-module symbol). It also **owns the device-wide FLR** (`reset.c`): a full reset resets every IP block (GFX *and* DCN) and wipes VRAM, so the base does the cold FLR once at attach (before children) and runs the coordinator `amd_device_reset` for the userspace recovery path — bracketing the FLR with each IP's prepare/restore hooks (`amd_reset_register`, same softc-pointer pattern), so a device-lost reset re-arms the display instead of killing it.
- `gpu/atrium_gpu_amd.ko` — attaches to the `atrium_gpu_amd` child; `sc = device_get_softc(device_get_parent(dev))`; owns `/dev/atrium-gpu0`. Registers `amd_eop_handler` for `IH_CAUSE_EOP` (fence retire); **has no role in vblank**. Resets only its own block via `amd_grbm_soft_reset` (GRBM_SOFT_RESET) — it **never FLRs the device** (so a gpu hot-swap leaves the display running); the userspace `gpu_reset` ioctl routes through the base's `amd_device_reset` coordinator. `MODULE_DEPEND` on the base.
- `display/atrium_gpu_amd_display.ko` — attaches to the `atrium_gpu_amd_display` child; owns `/dev/atrium-display0` with its own decoupled ABI (`atrium_display_abi.h`, `'D'` ioctl namespace: ENUM/MODES/SET_MODE/PAGE_FLIP/STATUS + CONFIG/USBC/MST/DPTRAIN). Registers `amd_display_vblank_handler` for `IH_CAUSE_VBLANK` (fires the `EVFILT_READ` knote on the display fd — userspace waits on vblank via kqueue, retiring `WAIT_VBLANK`). `MODULE_DEPEND` on the base, loadable independently of the gpu module — **including the vblank interrupt path**: `vblank_nogpu` proves vblanks still arrive via kqueue with `atrium_gpu_amd` unloaded (`/dev/atrium-gpu0` absent). The scanout buffer crosses from the GPU dma-buf-style: the GPU's `BO_EXPORT_SCANOUT` turns a VRAM BO into a plain `{vram_offset,size}`, which the compositor hands to the display's SET_MODE/PAGE_FLIP — the display imports VRAM by absolute offset and never touches the GPU's BO table (no cross-module bind). MODES decodes the EDID's Detailed Timing Descriptors (mirrors the model's `edid.rs`).
...
**Module-ownership rule (and the audit that enforced it):** a *device-global* resource or action (one that spans GFX + DCN, or outlives any one IP block) belongs to the **base**; a per-block one belongs to its IP module. Four defects/gaps of this class were found and **all fixed**: the IH ring/ISR (`ih.c`) and the device FLR (`reset.c`) — both were gpu-owned and could disturb the display behind its back; **display energy federation** — the display now registers its own member `"display0"` (`amd_display_energy_*` over `regDISP_POWER_*`, a real scanout+PSR power model in gpusim's `display.rs`) alongside the gpu's `"gpu0"`, observable via `sysctl kern.sched.energy_members`; and the **VRAM allocator** — the bump cursor + carve moved to `base/vram.c` (`amd_vram_alloc`) since VRAM is a device resource any IP block may use. Register namespaces are cleanly separated (gpu never touches `regDISP_*` and vice-versa). The driver-ownership audit is complete.
...
(The loader honors `MODULE_DEPEND`, loading the base before the gpu + display modules.) After boot the device tree is `atrium_gpu_amd_pci0 → {atrium_gpu_amd0, atrium_gpu_amd_display0}` with `/dev/atrium-gpu0` + `/dev/atrium-display0` present and no `devctl` — exactly how a shipped vendor driver binds.

#### Driver binding: preload vs devmatch (selective load)

Long-term we want `devmatch`/devd to load only the vendor base whose PCI ID matches the GPU present, instead of preloading every vendor's trio. The base now carries `MODULE_PNP_INFO("U16:vendor;U16:device", pci, atrium_gpu_amd_pci, …)` for `0x1002:0x7550`; `kldxref` folds it into `/boot/modules/linker.hints`, and `devmatch_enable="YES"` is the default. Findings from wiring this up (2026-06-21):

- **`vgapci` is built-in but does *not* claim the gpusim device.** After `devctl detach atrium_gpu_amd_pci0` the slot shows `none0@pci0:0:6:0` (driver `none`), not vgapci — so on this model the VGA-class device sits *unattached* without our driver, which is exactly the state `devmatch` autoload wants. (A real GPU may differ; verify per board.)
- **The `MODULE_PNP_INFO` discovery is real; the per-vendor *base* is the unit devmatch selects.** The gpu + display child modules attach to the base's *children* (bus `atrium_gpu_amd_pci`), not to `pci`, so devmatch (PCI-level) won't autoload them — they travel with their base (today: preloaded; cleaner: a packaging/dep that pulls them when the base instantiates). To test devmatch autoload end-to-end you must boot with the base *not* preloaded (devmatch only loads modules that aren't already in the kernel).
- ⚠️ **Attach is not re-entrant — `devctl detach` then re-attach panics** in `atrium_amd_attach` (the gpu child). Detach doesn't fully undo attach (energy_member register, MSI-X, GMC/IH state), so the re-probe faults → `db>`. This blocks the runtime-bind / forced-handoff path (#2/#3 below) and must be fixed first: make detach unwind everything attach set up, and attach idempotent. Until then, only a **fresh boot** binds cleanly.

For a display-class GPU the splash and the GPU-driver bind live on *different* framebuffers (splash = `vt_efifb`/firmware GOP; vendor driver = the GPU's own scanout), so the natural model is: efifb shows the boot splash, the vendor driver binds (preload now, devmatch once the child-module packaging + attach re-entrancy land), and the *visible* handoff (efifb → frescod scanout) is the separate splash→frescod switchover. The driver-bind and the display-ownership handoff are different layers.

**MSI-X delivery (resolved twice over):** (1) the post-split "MSI-X no delivery" from `pci_alloc_msix` being called by the non-owning gpu child — fixed by moving MSI-X allocation + vector routing into the **base** module (device owner); the gpu child only `bus_alloc`s vector 0 + hooks its ISR. (2) A deeper one: the QEMU device's **doorbell BAR was 0x1000** (one page), but the driver puts the GFX doorbell at page 0 and the **COMPUTE doorbell at page 1 (offset 0x1000)** — so a compute-doorbell write fell past the BAR end and corrupted the adjacent **MSI-X table**, clobbering vector 0's message address. That single bug caused the *entire* "pre-existing" failure cluster: `compute [0 0 0 0]` + `isolation` (compute doorbell never reached the model) AND `syncobj`/`cross_queue` (every interrupt after the first misrouted). Fixed by sizing the doorbell BAR to 4 MiB (`GPUSIM_DOORBELL_BAR_SIZE`, matching the model's advertised BAR2). **`atrium_gpu_test` now passes end to end ("ALL OK")** on a fresh boot. (Diagnosis path: instrumented the GICv3 ITS `do_process_its_cmd` to find gpusim's devid=56 translated only once, then printed the MSI-X message in the device to catch the address being overwritten with doorbell wptr values 0xb/0x7.)
...
- **Live `kldunload`/`kldload` of `gpu.ko`/`display.ko` now works** (the child-detach re-entrancy fix, commit `439d8ee`). It used to panic — `detach` read the wrong (child) softc, leaked the cdev, and the next `kldload`'s `make_dev` hit the stale node and panicked. Fixed: detach uses the base's shared softc, fully unwinds, and attach uses `MAKEDEV_CHECKNAME`. So for dev iteration on the gpu/display modules you can hot-swap live: `kldunload atrium_gpu_amd && cp …/atrium_gpu_amd.ko /boot/modules/ && kldload atrium_gpu_amd` — **provided no fds are open** (detach returns `EBUSY` while a BO/VM is live; stop frescod/tests first). This is the same path as the driver hot-swap (`docs/spec/gpu-driver-hotswap.md` Protocol A/B); verified 3× gpu + 2× display swap with `amd_smoke` green after. The **base** (`pci.ko`) still can't be live-swapped (it owns the BARs/softc) — reboot for base changes.
