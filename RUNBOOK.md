...
## 0. Quick start — build everything from scratch

`scripts/bootstrap-atrium.sh` takes a bare checkout and a Mac to a bootable VM
plus a tree of built artifacts. It is the answer to "I just cloned this, now
what" and the thing to keep current as the build evolves — everything below in
this file is the manual, per-component version of what it automates.

```sh
sh scripts/bootstrap-atrium.sh              # run every phase that is not done
sh scripts/bootstrap-atrium.sh --list       # phases and their state
sh scripts/bootstrap-atrium.sh --only kmod  # one phase
...
| phase | does | roughly |
|---|---|---|
| `preflight` | checks macOS/Apple Silicon, brew, Xcode CLT, llvm, bmake, python3, rustup, 40G free | instant |
| `clone` | clones `freebsd-src` (**atrium-os** branch) and the patched QEMU | ~15 min cold |
| `fetch` | downloads FreeBSD 16.0-CURRENT `base`/`kernel`/`src` .txz | ~5 min cold |
| `sysroot` | extracts headers, libs and `ld-elf.so.1` into `sysroot/` | ~1 min |
| `qemu` | builds `qemu-system-aarch64` with HVF | 20–45 min cold |
| `kernel` | `kernel-toolchain` + `buildkernel`, **on the Mac** | ~6 min + ~2.5 min |
| `kmod` | cross-builds `tessera_fs.ko` | seconds |
| `userspace` | cross-builds portcullisd/portcullis/opifex/frescod/forum-{wm,bar,dock} | ~1 min |
| `image` | creates `vm/vm.qcow2` from FreeBSD's published arm64 image, the four aux volumes, and the EFI pflash pair | ~5 min cold |
| `stage` | assembles `dist/` + generates `dist/install-atrium.sh` | instant |

Warm re-run with everything cached: **~35 s**. Phases are idempotent and stamped
in `.bootstrap-state/`; per-phase logs land in `.bootstrap-state/logs/`.

**It never boots anything.** It ends with `dist/` populated and a printed summary.

### Reusing what you already have

Every input location is overridable, so an existing dev machine never re-clones
or re-downloads. Anything already at the default path is reused automatically;
these are for when it lives somewhere else:

| var | meaning |
|---|---|
| `ATRIUM_FBSD_SRC` | existing `freebsd-src` checkout (must be on `atrium-os`) |
| `ATRIUM_QEMU_DIR` | existing patched qemu tree |
| `ATRIUM_QEMU_BIN` | an already-built `qemu-system-aarch64` — **skips the qemu build entirely** |
| `ATRIUM_TARBALLS` | directory already holding the .txz sets |
| `ATRIUM_SYSROOT` | an already-extracted sysroot |
| `ATRIUM_OBJDIR` | kernel objdir (large — worth a fast disk) |
| `ATRIUM_VM_DIR` | where VM disks and firmware live (default `vm/`) |
| `ATRIUM_STATE` / `ATRIUM_DIST` | phase stamps+logs / staged artifacts |
| `ATRIUM_VM_IMAGE_URL` | override the FreeBSD VM image URL when a snapshot rotates |

### Getting the artifacts into the guest

macOS cannot mount the guest's UFS, so host-built artifacts are **not** injected
into the image. They go in over 9p on first boot:

```sh
sh scripts/run-vm.sh
# then inside the guest:
kldload p9fs && mount -t p9fs -o trans=virtio bsd_share /mnt/host
sh /mnt/host/dist/install-atrium.sh
```

`install-atrium.sh` is generated by the `stage` phase (so it always matches what
was actually staged): installs the kernel to `/boot/atrium/kernel`, the module,
the userspace into `/usr/local/bin`, and sets `nullfs_load="YES"`.

### What it verifies, and why

Exit status 0 is never accepted as evidence on its own — this tree has produced
builds that "succeeded" while doing nothing (a sync that silently no-op'd, a
`.ko` linked before its `.o` compiled). Each phase proves its own output:

- **kernel** — the binary must be ARM aarch64 **and carry Laminar symbols**. No
  Laminar means stock FreeBSD was built instead of the `atrium-os` fork.
- **kmod** — gated on `nm` finding `blake3_hash_many_neon` and
  `tessera_vop_mknod`, because a parallel make here *has* linked a `.ko` before
  one of its objects compiled and still exited 0.
- **fetch** — each `.txz` must really be an xz archive (a 404 page saved under
  the right filename is the failure mode).
- **stage** — refuses to ship anything `file(1)` does not call ARM aarch64;
  copying in a host build by mistake otherwise only fails inside the guest.
- **image** — the EFI pflash images must be exactly 67108864 bytes.

### Gotchas the script encodes

- **`make.py`'s bmake bootstrap fails on macOS.** It runs bmake's unit tests and
  `cmd-interrupt` / `deptgt-interrupt` fail on Darwin signal semantics. The
  *binary* builds fine, so the script builds it, installs it by hand, writes the
  `.make-py-config` stamp `make.py` compares against, and then asserts `make.py`
  does not print "Bootstrapping bmake" — a silent retry is what made
  `kernel-toolchain` fail.
- **`--with-machine=amd64` is correct.** It is hardcoded upstream in
  `tools/build/make.py` (with its own `# TODO?`) and configures *bmake itself*,
  not the target. It must be mirrored byte for byte because `make.py` compares
  the string verbatim; "fixing" it to aarch64 makes every run re-bootstrap.
...
Booting a from-scratch VM end to end, and the 613 MB VM-image download, have not
been exercised on a machine without an existing `vm.qcow2`. There is also no
`desktop` phase yet for the jailed Forum apps.
...
~/src/bsd/
  ...
  dist/                   # staged build output from bootstrap-atrium.sh (gitignored)
  .bootstrap-state/       # bootstrap phase stamps, logs, kernel objdir (gitignored)
  scripts/
    bootstrap-atrium.sh   # §0 — clone/build/stage everything from scratch
  ...
  fresco-kmod/            # fresco.ko kernel module source (built IN-VM; the
                          #   tessera kmod cross-builds on the host — see §4)
...
> **The kernel and the Tessera module now cross-build ON THE MAC.** See
> "FreeBSD kernel + kmod on the host" below. The older "must be built in-VM"
> guidance in the `fresco.ko` subsection is about KBI matching, not about the
...
driver and explicitly supports macOS (it auto-detects a Homebrew llvm).

```
kernel-toolchain   372s   (one-time per objdir)
buildkernel        140s   GENERIC, -j10     vs ~390s median IN-VM   = 2.8x faster
tessera_fs.ko        2s   full rebuild, -j8
```

```sh
cd freebsd-src/usr/src
# kernel-toolchain FIRST — without it buildkernel dies on "config: command not found"
env MAKEOBJDIRPREFIX=<objdir> python3 tools/build/make.py \
    TARGET=arm64 TARGET_ARCH=aarch64 -j10 kernel-toolchain
env MAKEOBJDIRPREFIX=<objdir> python3 tools/build/make.py \
    TARGET=arm64 TARGET_ARCH=aarch64 -j10 buildkernel KERNCONF=GENERIC

# kmod, out of tree, through buildenv (gives it the cross toolchain + bootstrap PATH)
env MAKEOBJDIRPREFIX=<objdir> python3 tools/build/make.py TARGET=arm64 TARGET_ARCH=aarch64 \
    buildenv BUILDENV_SHELL="/bin/sh -c 'cd <kmoddir> && make -j8 SYSDIR=<src>/sys'"
```

**Case-insensitive APFS is NOT a blocker** — the src tree has zero case
collisions (`git ls-files | tr A-Z a-z | sort | uniq -d` is empty). FreeBSD
scrubbed those from src years ago.

Gotchas:

- **`MAKEOBJDIRPREFIX` must be an environment variable**, never a command-line or
  `make.conf` variable — the makefile enforces this and errors out.
- **`buildenv` strips `PATH`** to the bootstrap dirs only, so `tail`, `grep` etc.
  are unavailable inside `BUILDENV_SHELL`. Redirect, don't pipe.
- **The bmake bootstrap fails on macOS** — see §0 for the workaround; the binary
  builds fine, only bmake's own SIGINT unit tests fail.

**Verify a cross-built module before trusting it.** Gate on `llvm-nm -u` (135
undefined symbols, all kernel-provided, is normal) and on a symbol from each
object you expect — a parallel make here has linked a `.ko` before one of its
`.o` files compiled and still exited 0. The real proof is end to end: the
host-built `tessera_fs.ko` kldloads in the VM, mounts the 25G Tessera devroot,
and a `sha256` of a binary read back through it matches the host file exactly.

**The NEON translation unit needed a real fix (`6bf7ccc`).** `b3_blake3_neon.c`
needs the compiler's `arm_neon.h`, and the old rule got it by dropping
`-nostdinc` — the pattern FreeBSD itself uses in
`sys/modules/armv8crypto/Makefile`. That only works when the compiler *is* the
target's. The two toolchains disagree about the resource dir:

| | `arm_neon.h` | `stdint.h` | result |
|---|---|---|---|
| FreeBSD clang 19 | present | **absent** | include not found |
| Homebrew clang 21 | present | present | typedef collision, `-Werror`, dead |

So neither including nor excluding the resource dir works on both. Fix is
`atrium-tessera/kmod/neon-compat/stdint.h`, forwarding to `<sys/stdint.h>`,
placed ahead of `-idirafter <resource-dir>`. **Always verify a kmod Makefile
change BOTH ways** — the first attempt fixed the host and broke the in-VM build.

### opifex installs bundles from a sysroot

`opifex` used to require the target, because it shelled out to `ldd(1)` and
copied libraries from the running host's `/lib`. It now walks `DT_NEEDED`
directly and reads from `--sysroot` (default `/`, so on-target behaviour is
unchanged):

```sh
opifex install <bundle-dir> --allow-unsigned --root <atrium-root> \
               --sysroot ~/src/bsd/sysroot
```

The walk **recurses**, and that matters: `ldd` prints the whole transitive
closure while `DT_NEEDED` names only direct dependencies. `portcullisd`'s
`DT_NEEDED` lists 4 libraries but `ldd` reports 5 — `libsys.so.7` arrives via
`libc`. A missing library is now a hard error; it used to return success and
emit a bundle with no libraries in it, which only failed later inside a jail.

Note the sysroot needs `libexec/ld-elf.so.1`, which the "headers+libs only"
extract in §1 omits:
```sh
tar -xf tarballs/base.txz -C sysroot ./libexec/ld-elf.so.1
```
