/*
 * b3_blake3_neon.c — vendored NEON backend for the BLAKE3 reference C
 * implementation (upstream 1.8.5, CC0-1.0 / Apache-2.0 — permissive, so it
 * satisfies the no-copyleft-in-runtime policy). Taken verbatim from the copy
 * already in FreeBSD base (contrib/llvm-project/.../BLAKE3/blake3_neon.c);
 * only the include is repointed at our b3_ prefixed header.
 *
 * Compiled ONLY when BLAKE3_USE_NEON=1. The kernel build leaves it 0 (see
 * b3_compat.h) — enabling kernel SIMD additionally requires fpu_kern_enter
 * regions, sliced to bound preemption latency (task #89).
 */
