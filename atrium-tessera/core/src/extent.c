 ...
 *
 * Phase 1 ships the in-memory data structure + algorithm only. The
 * on-disk B+tree backing lands once the B+tree primitives are in (the
 * persistent root sector is then a snapshot of this in-memory set on
 * tx_commit, COW-rebuilt from leaf up).
 *
 * Representation: a sorted dynamic array of (start_sector,
 * length_sectors), kept invariant:
...
{
	/* Phase 1: io is unused — in-memory only. NULL io / 0 root means
	 * "empty allocator", suitable for mkfs-time seeding via free(). */
	(void)io; (void)free_root_sector;
